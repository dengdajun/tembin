...
public class PayPalServiceImpl implements PayPalService {
    ...
    public UsercontrollerPaypalAccount getADevSin(Long paypalId,String... source){
        ...
        if (!"1".equals(uspa.getSfCheck()) && (source==null || source.length<=0)){return null;}
    ...
    public Map getTransactionDetails(Map map) throws Exception {
       ...
       /* Long paypalId= (Long) map.get("paypalId");*/
        ...
        if(ObjectUtils.isLogicalNull(paypalAccounts)){
            return map1;
        }
        ...
        if(StringUtils.isNotBlank(res)){
            ...
            map1.put("xml",PaypalxmlUtil.getTransactionDetailsXML(uspa,paypalVO));
        }else{
            map1.put("res",res);
    ...
    @Override
    /**获取paypal账户列表*/
    public List<UsercontrollerPaypalAccount> queryPayPalListTo(Map map,Page page){
        List<UsercontrollerPaypalAccount> paypalAccounts = aboutPaypalMapper.queryPayPalsByUserIdList(map,page);
        return paypalAccounts;
    }
    ...
    public List<BaseTjReportQuery> selectPayPalReportList(String type){
        ...
        if("1".equals(type)){//当天反馈信息统计
            ...
            m.put("endDate",DateUtils.formatDate(DateUtils.turnToWeekEnd(date.getTime())));
        ...
        if (!ObjectUtils.isLogicalNull(ebayAccountExtendList)){
            List<Long> longList=new ArrayList<>();
            for (UsercontrollerEbayAccountExtend extend:ebayAccountExtendList){
                longList.add(extend.getId());
            }
            m.put("ebays",longList);
        }
    ...
    public void addPayPalAccount(Map map){
        ...
        UsercontrollerPaypalAccountExample example = new UsercontrollerPaypalAccountExample();
        example.createCriteria().andPaypalAccountEqualTo(paypalAccount.getPaypalAccount());
        List<UsercontrollerPaypalAccount> paypalAccounts = usercontrollerPaypalAccountMapper.selectByExample(example);
        Asserts.assertTrue(ObjectUtils.isLogicalNull(paypalAccounts),"已经存在相同的paypal帐号！");
    ...
    public Map<String, String> refundTransactionPartial(Map map) throws Exception {
        ...
        HttpClient h= HttpClientUtil.getHttpsClient();
        String res = HttpClientUtil.post(h,PAYAPL_API_URL, PaypalxmlUtil.getRefundTransactionPartial(paypalAccount, transactionID, money,currency));
        Element element=PaypalxmlUtil.getSpecElement(res,"Body","RefundTransactionResponse","Ack");
        String flag=element.getTextTrim();
        if(!StringUtils.isNotBlank(flag)){
            return null;
