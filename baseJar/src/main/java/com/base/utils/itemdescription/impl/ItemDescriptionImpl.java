...
public class ItemDescriptionImpl implements com.base.utils.itemdescription.IItemDescription {
    ...
    public String getPaDescription(String des, TradingItemWithBLOBs tradingItem,Item item){
        try {
            if(StringUtils.containsIgnoreCase(des,"http://www.inkfrog.com")){//青蛙模板
                org.jsoup.nodes.Document doc = org.jsoup.Jsoup.parse(des);
                org.jsoup.select.Elements content = doc.getElementsByAttributeValue("class", "r_des1");
                if (content != null && content.size() > 0) {
                    tradingItem.setDescription(content.get(0).toString());
                ...
                return "";
            }else if(StringUtils.containsIgnoreCase(des,"http://www.channeladvisor.co.uk")){//通途模板
                org.jsoup.nodes.Document doc = org.jsoup.Jsoup.parse(des);
                org.jsoup.select.Elements elements = doc.select("div[style=border:1px solid #939191;border-top:0;border-bottom:0;font-size:14px;line-height:24px;padding:0 10px;]");
                if(elements!=null&&elements.size()>0){
                    tradingItem.setDescription(elements.get(0).toString());
                    this.tradingItemMapper.updateByPrimaryKeySelective(tradingItem);
                }else{
                    tradingItem.setDescription("");
                    this.tradingItemMapper.updateByPrimaryKeySelective(tradingItem);
