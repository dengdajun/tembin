                   ...
                   /* resMap = addApiTask.exec(d, xml, "https://api.ebay.com/ws/api.dll");*/
                    ...
                    if ("Success".equalsIgnoreCase(ack)||"Warning".equalsIgnoreCase(ack)) {
                        ...
                        for (TradingOrderGetOrders order : orders) {
                            ...
                            Date date11=DateUtils.buildDateTime(2015,0,29,0,0,0);
                            Date date22=DateUtils.buildDateTime(2015,0,24,0,0,0);
                            if (addmessages != null && addmessages.size() > 0) {
                                ...
                                if(autoMessageAndOrders!=null&&autoMessageAndOrders.size()>0){
                                    for(TradingAutoMessageAndOrder autoMessageAndOrder:autoMessageAndOrders){
                                        ...
                                        if((partners==null||partners.size()==0)&&StringUtils.isNotBlank(trackingnumber)&&modifyDate.after(date22)){
                           ...
                           // Map<String, String> accountresmap = addApiTask.exec(ds, accountxml, "https://api.ebay.com/ws/api.dll");
                            ...
                            if ("Success".equalsIgnoreCase(sellerAck)) {
                                ...
                                for (TradingOrderGetSellerTransactions list : lists) {
                                    List<TradingOrderGetSellerTransactions> transactionseList = iTradingOrderGetSellerTransactions.selectTradingOrderGetSellerTransactionsByTransactionId(list.getItemid(),list.getTransactionid());
