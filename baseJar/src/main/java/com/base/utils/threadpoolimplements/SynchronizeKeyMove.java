...
public class SynchronizeKeyMove implements ThreadPoolBaseInterFace {
    ...
    @Autowired
    public SiteMessageService siteMessageService;
    ...
    public <T> void doWork(String ebpRes, T... t) {
        ...
        String ebayid = String.valueOf(xs[5]);
        String ebpAck = null;
        try {
            ...
            if ("Success".equalsIgnoreCase(ebpAck) || "Warning".equalsIgnoreCase(ebpAck)) {
                ...
                for (Item item : li) {
                    ...
                    if(likml==null||likml.size()==0){
                        KeyMoveListWithBLOBs kml = new KeyMoveListWithBLOBs();
                        kml.setCreateTime(new Date());
                        kml.setItemId(item.getItemID());
                        kml.setUserId(Long.parseLong(userid));
                        kml.setEbaytoken(token);
                        kml.setTaskFlag("0");
                        kml.setSiteId(siteids);
                        kml.setPaypalId(ebayid);
                        kml.setProgressId(tp.getId());
                        kml.setSku(item.getSKU());
                        this.keyMoveListMapper.insertSelective(kml);
                        is++;
                    }
                }
                if(is==0){
                    TaskMessageVO tmv = new TaskMessageVO();
                    tmv.setMessageTitle("一键搬家通知消息");
                    tmv.setMessageTo(Long.parseLong(userid));
                    tmv.setMessageFrom("system");
                    tmv.setMessageContext("一键搬家执行成功，但未发现需要搬家的数据，请核对提交的数据是否正确！");
                    tmv.setMessageType(SiteMessageStatic.LISTING_KEY_MOVE_MESSAGE_TYPE);
                    siteMessageService.addSiteMessage(tmv);
