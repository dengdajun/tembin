 ...
 * 速卖通用于存放较长时限数据的缓存
 */
public class Seven7HStoreDataSupport extends CacheBaseSupport {
    ...
    public static <T> void pushData(String key,T t){
        ...
        Element element=new Element(key+"_7h",t);
        cache.put(element);
    }
    ...
    public static <T> T pullData(String key){
        Cache cache =cacheManager.getCache(TEMP_DATA_CACHE);
        Element element=cache.get(key+"_7h");
