 ...
 * session缓存管理支持,注意，该缓存只清除一小时之内没有用到的缓存，不是按时间强制删除
 */
@Service
public class SessionCacheSupport extends CacheBaseSupport {
    ...
    /**获取当前在线人员*/
    public static List<SessionVO> getOnlineUser(){
        Cache cache =cacheManager.getCache(sessionName);
        List keys = cache.getKeys();
        List<SessionVO> sessionVOList=new ArrayList<>();
        if (!ObjectUtils.isLogicalNull(keys)){
            for (Object skey:keys){
                Element element=cache.get(skey);
                if (element!=null&&SessionVO.class.equals(element.getObjectValue().getClass())){
                    sessionVOList.add((SessionVO)element.getObjectValue());
                }
            }
        }
