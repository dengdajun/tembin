...
public class DataDictionarySupport extends CacheBaseSupport{
    ...
    public static final String SHIPPINGTOADDRESS ="ShippingToAddress";//运输到国家
    ...
    public static PublicDataDict getPublicDataDictionaryByItemIDs(String itemid,String type){
        ...
        if (ObjectUtils.isLogicalNull(tradingDataDictionaries)){return null;}
    ...
    /**判断运输方式是否有重复*/
    public static List<String> checkShipingIsExit(DataDicShipingQueryParamVO vo)throws Exception{
        if (ObjectUtils.isLogicalNull(vo.getOrgID())||
                ObjectUtils.isLogicalNull(vo.getShipingString())||
                ObjectUtils.isLogicalNull(vo.getSiteID())){
            throw new Exception("判断需要的参数不能为空！");
        }
        ITradingDataDictionary dictionary = (ITradingDataDictionary) ApplicationContextUtil.getBean(ITradingDataDictionary.class);
        return dictionary.checkShipingIsExit(vo);
    }
}
