...
public class PriceTrackingAutoPricingTaskRun extends BaseScheduledClass implements Scheduledable {
    ...
    public void run(){
        ...
        if(priceTrackings!=null&&priceTrackings.size()>0){
            TradingPriceTrackingAutoPricing priceTracking=priceTrackings.get(0);
            TradingListingData listingData=iTradingListingData.selectById(priceTracking.getListingdateId());
            List<TradingPriceTrackingPricingRule> rules=iTradingPriceTrackingPricingRule.selectTradingPriceTrackingPricingRuleByAutoPricingId(priceTracking.getId());
            List<TradingPriceTrackingAutoPricing> priceTracking1=new ArrayList<>();
            for(TradingPriceTrackingPricingRule rule:rules){
                TradingPriceTracking tracking=iTradingPriceTracking.selectPriceTrackingById(rule.getPricetrackingId());
                Double listPrice=listingData.getPrice();
                if(StringUtils.isNotBlank(tracking.getCurrentprice())){
                    Double trackPrice= Double.valueOf(tracking.getCurrentprice());
                    if("priceLowerType".equals(rule.getRuletype())){
                        if(listPrice<trackPrice){
                            priceTracking1.add(priceTracking);
                        }
            ...
            if(priceTracking1!=null&&priceTracking1.size()>0){
                syschronizePriceTrackingAutoPricing(priceTracking1);
            }
        }
                ...
                if(prices!=null&&prices.size()>0){
                    ...
                    if(flag){
                        ...
                        if(StringUtils.isNotBlank(newPrice)){
                            ...
                            //---------------------------
                            Integer pricecount=autoPricing.getPricecount();
                            autoPricing.setPricecount(pricecount+1);
                            try {
    ...
    private Map<String,Double> syschronizeGetCompetitorPrice(List<TradingPriceTrackingPricingRule> rules){
        ...
        //headers.add(new BasicHeader("X-EBAY-API-APP-ID", "chengdul-6dfe-4b1b-905c-41b1bd716d72"));//sandpoin-23af-4f47-a304-242ffed6ff5b
