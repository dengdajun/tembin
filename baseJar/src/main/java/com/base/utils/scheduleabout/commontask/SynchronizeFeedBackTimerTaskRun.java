...
public class SynchronizeFeedBackTimerTaskRun extends BaseScheduledClass implements Scheduledable {
    ...
    public void synchronizeFeedBack(List<TaskFeedBack> taskFeedBacks){
        ...
        try{
            for(TaskFeedBack taskFeedBack:taskFeedBacks){
                UsercontrollerEbayAccount ebayAccount= iUsercontrollerEbayAccount.selectById(taskFeedBack.getEbayId());
                String ebayName="";
                if(ebayAccount!=null){
                    if (StringUtils.isBlank(ebayAccount.getEbayAccount())){
                        ebayName=ebayAccount.getEbayName();
                    }else {
                        ebayName=ebayAccount.getEbayAccount();
                    }

                }else {
                    logger.error("feedbackEbayname不能为空！id:"+taskFeedBack.getEbayId());
                    continue;
                }
                String res = this.cosPostXml(taskFeedBack,1,ebayName);
                ...
                if("Success".equals(ack)||"Warning".equalsIgnoreCase(ack)){
                    ...
                    for(int i = pages;i>0;i--){
                        ...
                        for(TradingFeedBackDetail feedBackDetail:litfb){
                            ...
                            if(feedBackDetail.getTransactionid()!=null){
                                addmessages=iTradingOrderAddMemberMessageAAQToPartner.selectTradingOrderAddMemberMessageAAQToPartnerByTransactionId(feedBackDetail.getTransactionid(),feedBackDetail.getItemid(),3,null,"true");
                            }else{
                                addmessages=iTradingOrderAddMemberMessageAAQToPartner.selectTradingOrderAddMemberMessageAAQToPartnerByItemIdAndSender(feedBackDetail.getItemid(),3,feedBackDetail.getCommentinguser(),ebayName,"true");
                            }
                          ...
                          /*  TaskMessageVO taskMessageVO = new TaskMessageVO();
                            ...
                            if("Seller".equals(feedBackDetail.getRole())) {
                                if ("Positive".equals(feedBackDetail.getCommenttype())) {
                                    partners = iTradingAutoMessage.selectAutoMessageByType("goodReviewAutoMessage", taskFeedBack.getUserid());
                                } else if ("Neutral".equals(feedBackDetail.getCommenttype())) {
                                    partners = iTradingAutoMessage.selectAutoMessageByType("mediumReviewAutoMessage", taskFeedBack.getUserid());
                                } else if ("Negative".equals(feedBackDetail.getCommenttype())) {
                                    partners = iTradingAutoMessage.selectAutoMessageByType("badReviewAutoMessage", taskFeedBack.getUserid());
                                }
                            }
                            if(partners.size()==0&&"Positive".equals(feedBackDetail.getCommenttype())){
                                List<PublicSitemessage> list1=siteMessageService.selectPublicSitemessageByMessage("synchronize_feed_back_timer_FAIL", "评价定时任务:没有(收到买家的正评)自动消息"+ebayName);
                                /*if(list1==null||list1.size()==0){
                                    taskMessageVO.setMessageContext("没有对应的自动消息,"+ebayName+"请先创建(收到买家的正评)自动消息");
                                    taskMessageVO.setOrderAndSeller("评价定时任务:没有(收到买家的正评)自动消息"+ebayName);
                                    siteMessageService.addSiteMessage(taskMessageVO);
                                }*/
                            }else if(partners.size()==0&&"Neutral".equals(feedBackDetail.getCommenttype())){
                                List<PublicSitemessage> list1=siteMessageService.selectPublicSitemessageByMessage("synchronize_feed_back_timer_FAIL", "评价定时任务:没有(收到买家的中评)自动消息"+ebayName);
                               /* if(list1==null||list1.size()==0){
                                    taskMessageVO.setMessageContext("没有对应的自动消息,"+ebayName+"请先创建(收到买家的中评)自动消息");
                                    taskMessageVO.setOrderAndSeller("评价定时任务:没有(收到买家的中评)自动消息"+ebayName);
                                    siteMessageService.addSiteMessage(taskMessageVO);
                                }*/
                            }else if(partners.size()==0&&"Negative".equals(feedBackDetail.getCommenttype())){
                                List<PublicSitemessage> list1=siteMessageService.selectPublicSitemessageByMessage("synchronize_feed_back_timer_FAIL", "评价定时任务:没有(收到买家的负评)自动消息"+ebayName);
                              /*  if(list1==null||list1.size()==0){
                                    taskMessageVO.setMessageContext("没有对应的自动消息,"+ebayName+"请先创建(收到买家的负评)自动消息");
                                    taskMessageVO.setOrderAndSeller("评价定时任务:没有(收到买家的负评)自动消息"+ebayName);
                                    siteMessageService.addSiteMessage(taskMessageVO);
                                }*/
                            }
                            ...
                            feedBackDetail.setEbayAccount(ebayName);
                    ...
                    String errorCode=SamplePaseXml.getErrorCode(res);
                    logger.error("定时获取评价参数错误(错误码:"+errorCode+")ebayName("+ebayName+"):" + res);
                }
            ...
            logger.error("定时同步评价出错:"+e.getMessage(),e);
            ...
            if (partner.getStartuse() == 1) {
                ...
                if(order!=null){
                    ...
                    if (autoFlag) {
                        ...
                        if(!"shippedAutoMessage".equals(partner.getType())){
                            ...
                            ITradingOrderAddMemberMessageAAQToPartner iTradingOrderAddMemberMessageAAQToPartner=(ITradingOrderAddMemberMessageAAQToPartner) ApplicationContextUtil.getBean(ITradingOrderAddMemberMessageAAQToPartner.class);
                            List<TradingOrderAddMemberMessageAAQToPartner> addMemberMessageAAQToPartners=iTradingOrderAddMemberMessageAAQToPartner.selectTradingOrderAddMemberMessageAAQToPartnerByAutoMessageIdAndOrderId(partner.getId(),order.getId(),"true");
                            if(addMemberMessageAAQToPartners==null||addMemberMessageAAQToPartners.size()==0){
                                List<TradingOrderAddMemberMessageAAQToPartner> iitems=iTradingOrderAddMemberMessageAAQToPartner.selectTradingOrderAddMemberMessageAAQToPartnerByAutoMessageIdAndBuyerAndItemid(partner.getId(),order.getBuyeruserid(),order.getItemid(),"true");
    ...
    public String cosPostXml(TaskFeedBack taskFeedBack,int pageNumber,String ebayName){
        //真实环境
        ...
        //Map<String, String> resMap= addApiTask.exec2(d, colStr, "https://api.sandbox.ebay.com/ws/api.dll");
        Map<String, String> resMap= addApiTask.exec2(d, colStr, commPars.apiUrl);
        String res=resMap.get("message");
        String r11 = resMap.get("stat");
        if(res==null||"".equals(res)||"fail".equalsIgnoreCase(r11)){
            /*TradingTaskXml ttx = new TradingTaskXml();
            ttx.setCreateDate(new Date());
            ttx.setTaskType("getFeedbackList");
            ttx.setXmlContent(colStr);
