...
public class SynchronizeGetMessagesTimerTaskRun extends BaseScheduledClass implements Scheduledable {
    ...
    public void synchronizeOrders(List<TaskGetMessages> TaskGetMessages){
        ...
        for(TaskGetMessages taskGetMessages:TaskGetMessages){
            ...
            d.setApiSiteid("0");
            d.setRepetitionMark("getMessages_"+taskGetMessages.getId());
            //真实环境
           /* UsercontrollerDevAccountExtend d=new UsercontrollerDevAccountExtend();
            ...
            while (messageFlag){
                ...
                /*if(list1==null||list1.size()==0){
                    ...
                    String errorCode="";
                    try {
                        errorCode=SamplePaseXml.getErrorCode(res);
                    } catch (Exception e) {
                        logger.error("解析出错:",e);
                    }
                    logger.error("调用Message API失败(错误码:"+errorCode+"):"+res);
                }else {
                    String ack = null;
                    try {
                        try {
                        ...
                        if ("Success".equalsIgnoreCase(ack) || "Warning".equalsIgnoreCase(ack)) {
                            ...
                            /*if (list1 == null || list1.size() == 0) {
                                ...
                                logger.error("获取定时Message有警告!(错误码:"+errorCode+")" + errors);
                            }
                    ...
                    d.setApiSiteid("0");*/
                                    ...
                                    List<TradingMessageGetmymessage> getmymessages = iTradingMessageGetmymessage.selectMessageGetmymessageByMessageId(ms.getMessageid(),taskGetMessages.getEbayid());
                            ...
                            String errorCode=SamplePaseXml.getErrorCode(res);
                            logger.error("定时获取Message参数错误1:(错误码:"+errorCode+")" + res);
                        }
                    } catch (Exception e) {
            ...
            while (messageFlag){
             ...
             /*        d=new UsercontrollerDevAccountExtend();
                    ...
                    d.setApiSiteid("0");*/
                            ...
                            String errorCode=SamplePaseXml.getErrorCode(res);
                            logger.error("定时获取Message参数错误:(错误码:"+errorCode+")" + res);
                        }
                    } catch (Exception e) {
                        TempStoreDataSupport.removeData("task_" + getScheduledType());
                        logger.error("定时同步消息每两分钟出错:" + e.getMessage(), e);
