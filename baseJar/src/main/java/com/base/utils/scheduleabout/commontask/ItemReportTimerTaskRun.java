    ...
    public void run() {
        ...
        if(liuu!=null&&liuu.size()>0){
            for(UsercontrollerUser uu:liuu){
                ...
                //本周销售比
                List<ListingItemReportQuery> weekListingSales = iTradingListingSuccess.selectListingItemSales("3","1",null,uu.getUserId()+"");
                //上周销售比
                List<ListingItemReportQuery> thatweekListingSales = iTradingListingSuccess.selectListingItemSales("4","1",null,uu.getUserId()+"");
                //本月销售比
                List<ListingItemReportQuery> monthListingSales = iTradingListingSuccess.selectListingItemSales("5","1",null,uu.getUserId()+"");
                //上月销售比
                List<ListingItemReportQuery> thatmonthListingSales = iTradingListingSuccess.selectListingItemSales("6","1",null,uu.getUserId()+"");
                ...
                for(int i=0;i<m.size();i++){
                    try {
                        ...
                        if(litlr!=null&&litlr.size()>0){
                            ...
                            }else if(m.get(i+"").equals("4")&&tlr.getDatatype().equals("4")){
                                tlr.setDay(dayListingSales.size()>0?dayListingSales.get(0).getTjNumber():"0.00");
                                tlr.setYesterday(yesterdayListingSales.size()>0?yesterdayListingSales.get(0).getTjNumber():"0.00");
                                tlr.setWeek(weekListingSales.size()>0?weekListingSales.get(0).getTjNumber():"0.00");
                                tlr.setWeek(thatweekListingSales.size()>0?thatweekListingSales.get(0).getTjNumber():"0.00");
                                tlr.setMonth(monthListingSales.size()>0?monthListingSales.get(0).getTjNumber():"0.00");
                                tlr.setThatmonth(thatmonthListingSales.size()>0?thatmonthListingSales.get(0).getTjNumber():"0.00");
