...
public class ListingItemDataTaskRun extends BaseScheduledClass implements Scheduledable {
    ...
    public void run() {
        ...
        for(int i =0 ;i<liue.size();i++) {
            ...
            /*List<ItemQuery> liiq = iTradingItem.selectByEbayIdSiteList(ue.getId()+"");
            if(liiq!=null&&liiq.size()>0){
                for(ItemQuery iq:liiq){
                    String site = DataDictionarySupport.getTradingDataDictionaryByID(Long.parseLong(iq.getSite())).getName1();
                    List<ListingDataTask> lidk = iListingDataTask.selectByflag(site,ue.getEbayAccount());
                    if(lidk==null||lidk.size()==0) {
                        ListingDataTask ldt = new ListingDataTask();
                        ldt.setCreateDate(new Date());
                        ldt.setEbayaccountid(ue.getId());
