            ...
            if(liaad!=null&&liaad.size()>0){
                for(AutoAssessDetailWithBLOBs aad:liaad){
                    ...
                    if ("Success".equalsIgnoreCase(ack)||"Warning".equalsIgnoreCase(ack)) {
                        ...
                        try {
                            logger.error(aad.getSendUsername()+"发送给："+aad.getTargetUserid()+"评价；发送失败，返回的报文为："+res);
                        ...
                        //用户已经发过了，也会报错误信息
                        aad.setIsFlag("1");
                        if("55".equals(SamplePaseXml.getSpecifyElementTextAllInOne(res,"Errors","ErrorCode"))){
