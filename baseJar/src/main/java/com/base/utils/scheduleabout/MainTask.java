...
public class MainTask {
    ...
    public static final String SYNCHRONIZE_GET_ORDERS_BUYER_PAYPAL_ACOUNT="synchronize_get_orders_buyer_paypal_acount";//通过订单同步买家paypal账号
    public static final String CHECK_EBAY_STATUS_DESC="check_ebay_status_desc";//检查ebay,token账号是否过期，当前状态是什么，每天执行一次
    public static final String CHECK_EBAY_ACCOUNT_DESC="check_ebay_account_desc";//检查ebay账号信息，每天执行一次
    public static final String SET_EBAY_NOTICE_TASK="set_ebay_notice_task";//开通帐号的通知同能
    public static final String SYNCHRONOUS_SHIPPING_BASE_DATA="synchronous_shipping_base_data";//同步运输选项基础数据
    public static final String ItemSpecificsRun="ItemSpecificsRun";//同步自定义属性
    public static final String GET_PROMOTIONAL_SALE_DETAILS="GetPromotionalSaleDetails";//同步促销规则
    public static final String RE_LIST_ITEM="reListItem";//重新上架
    public static final String GetStoreCateGory="GetStoreCateGory";//同步店铺分类
    /**
     * 记录任务上次运行的时间
     */
    ...
    //@Scheduled(cron="0/10 * *  * * ?")
    public void doItEveryDay() throws Exception{
        if (isStartTimerTask==null) {
        ...
        //doList.add(CHECK_EBAY_STATUS_DESC);
        List<String> taskList=new ArrayList<String>();
        if("false".equalsIgnoreCase(isStartTimerTask.isStartTimerTask)){
            return;
        }else {
    ...
    public void mainMethodOther() throws Exception {
        ...
        doList.add(StaticParam.SYSTEM_VIP_USER_CHECK);
        doList.add(StaticParam.SYSTEM_VIP_USER_COST_FEE);
        doList.add(StaticParam.SYSTEM_SMS);
        doList.add(StaticParam.SYSTEM_SMS_STATUS);
        List<String> taskList=new ArrayList<String>();
