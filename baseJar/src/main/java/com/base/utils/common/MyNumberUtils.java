...
public class MyNumberUtils {
	...
	/** 金额除法,标量为2，舍入为四舍五入 */
	public static BigDecimal divide2(BigDecimal bigDecimal, BigDecimal other) {
		return bigDecimal.divide(other, SCALE_2, roundingMode_halfUp);
	}
    ...
    public static long randomLong(int digit) {//指定位数的随机数字
        if (digit >= 19 || digit <= 0)
    ...
    public static long randomLong(int minDigit, int maxDigit)  {//随机产生在指定位数之间的long型数据,位数包括两边的值
    ...
    /**指定给定范围的随机数*/
    public static long randomLongByFW(int min,int max){
        if (min > max) {
