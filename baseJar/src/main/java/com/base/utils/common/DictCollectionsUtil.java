...
public class DictCollectionsUtil {
    ...
    /**为ebay店铺分类组装菜单*/
    public static List<TradingStoreCategory> menuForEbayStoresCategory(List<TradingStoreCategory> storeCategories){
        if (ObjectUtils.isLogicalNull(storeCategories)){return null;}
        List<TradingStoreCategory> c=new ArrayList<>();
        List<TradingStoreCategory> c1=new ArrayList<>();
        //Map<Long,List<TradingStoreCategory>> c2=new HashMap<>();
        //Map<Long,List<TradingStoreCategory>> c3=new HashMap<>();

        c1=forMenuForEbayStoresCategory(storeCategories,-1L);
        if (ObjectUtils.isLogicalNull(c1)){return null;}
        //Iterator<TradingStoreCategory> iterator=c1.iterator();
        for (TradingStoreCategory ca:c1){//筛选出2级菜单
            //TradingStoreCategory ca=iterator.next();
            //stringBuffer.append(ca.getStoreCategoryName()).append("-");

            List<TradingStoreCategory> cc2=forMenuForEbayStoresCategory(storeCategories, ca.getId());
            if (ObjectUtils.isLogicalNull(cc2)){
                StringBuffer sb=new StringBuffer();
                sb.append(ca.getStoreCategoryName());

                TradingStoreCategory newC=new TradingStoreCategory();
                newC.setId(ca.getId());
                newC.setStoreCategoryName(sb.toString());
                newC.setStoreCategoryId(ca.getStoreCategoryId());
                c.add(newC);
                continue;
            }
            //c2.put(ca.getId(),cc2);
            for (TradingStoreCategory lcc2:cc2){
                List<TradingStoreCategory> cc3=forMenuForEbayStoresCategory(storeCategories,lcc2.getId());
                if (ObjectUtils.isLogicalNull(cc3)){
                    StringBuffer sb=new StringBuffer();
                    sb.append(ca.getStoreCategoryName()).append("-")
                            .append(lcc2.getStoreCategoryName());

                    TradingStoreCategory newC=new TradingStoreCategory();
                    newC.setId(lcc2.getId());
                    newC.setStoreCategoryName(sb.toString());
                    newC.setStoreCategoryId(lcc2.getStoreCategoryId());
                    c.add(newC);
                    continue;
                }
              //  c3.put(lcc2.getId(),cc3);

                for(TradingStoreCategory lcc3:cc3){
                    StringBuffer sb=new StringBuffer();
                    sb.append(ca.getStoreCategoryName()).append("-")
                            .append(lcc2.getStoreCategoryName()).append("-")
                            .append(lcc3.getStoreCategoryName());

                    TradingStoreCategory newC=new TradingStoreCategory();
                    newC.setId(lcc3.getId());
                    newC.setStoreCategoryName(sb.toString());
                    newC.setStoreCategoryId(lcc3.getStoreCategoryId());
                    c.add(newC);
                }
            }
        }
    ...
    private static List<TradingStoreCategory> forMenuForEbayStoresCategory(List<TradingStoreCategory> storeCategories, final Long parentId){

        Collection<TradingStoreCategory> x= Collections2.filter(storeCategories, new Predicate<TradingStoreCategory>() {
            @Override
            public boolean apply(TradingStoreCategory p) {
                if (p.getParentId()==parentId || p.getParentId().equals(parentId)){
                    return true;
                }
                return false;
            }
