...
public class OraclePageInterceptor implements Interceptor {
	...
	void processIntercept(Executor executor, final Object[] queryArgs) throws SQLException {
		...
		if(isPage) {
			...
			if(page.getTotalCount() == 0) {
				...
				//==========关于缓存记录页数的设置=========================================
				//算出sql+参数的md5值，相同的就不用再查，减轻重复查询的压力
				String parameterMapString=(parameter==null?"":parameter.toString()) ;
				String sqlMd5 = DigestUtils.md2Hex(oldSql+parameterMapString);
				Long temCountNum=TempStoreDataSupport.pullData(sqlMd5);
				if (temCountNum!=null){
					page.setTotalCount(temCountNum);
				}else {
					//==========关于缓存记录页数的设置=========================================
					...
					//==========关于缓存记录页数的设置=========================================
					TempStoreDataSupport.pushDataByTime(sqlMd5, totalCount, 1);
					//==========关于缓存记录页数的设置=========================================
	...
	private static String getCountSql(String sql) {
		/*String s= MyStringUtil.replaceFirstIgnoreCase(sql,"select ","select ");
		s=MyStringUtil.replaceFirstIgnoreCase(sql," from "," from ");
		String t1 = StringUtils.substringBetween(s, "select ", " from ");
		sql=sql.replace(t1," count(1) ");
		return sql;*/
