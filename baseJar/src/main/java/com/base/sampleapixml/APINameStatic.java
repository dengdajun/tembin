package com.base.sampleapixml;

/**
 * Created by Administrtor on 2014/8/7.
 * API名称
 */
public class APINameStatic {
    /**
     * 修改商品信息
     */
    public static final String ReviseItem = "ReviseItem";
    /**获取sessionID的api*/
    public static final String GetSessionID = "GetSessionID";
    /**获取运输方式ＡＰＩ*/
    public static final String GeteBayDetails = "GeteBayDetails";
    /**上传商品图片*/
    public static final String UploadSiteHostedPictures = "UploadSiteHostedPictures";
    /**获取token*/
    public static final String FetchToken = "FetchToken";
    /**获取帐号信息*/
    public static final String GetUser = "GetUser";
    /**取得不同站点的分类*/
    public static final String GetCategories = "GetCategories";
    /**修改库存*/
    public static final String ReviseInventoryStatus ="ReviseInventoryStatus";
    /**
     * 回复评价信息
     */
    public static final String RespondToFeedback="RespondToFeedback";
    /**
     * 添加固价商品
     */

    public static final String AddFixedPriceItem = "AddFixedPriceItem";
    /**
     * 通过关键字查询
     */
    public static final String FindItemsByKeywords = "findItemsByKeywords";
    /**
     * 拍买商品用
     */
    public static final String AddItem = "AddItem";
    /**
     * 商品下架
     */
    public static final String EndItem = "EndItem";

    public static final String EndItems = "EndItems";
    /**
     * 买家对卖家反馈信息
     */
    public static final String GetFeedbackRequest = "GetFeedback";
    /**
     * 获取接受消息
     */
    public static final String GetMyMessages = "GetMyMessages";

    /**
     * 回复消息
     */
    public static final String AddMemberMessageRTQ = "AddMemberMessageRTQ";

    /**获取商品类别属性*/
    public static final String GetCategorySpecifics="GetCategorySpecifics";
    /**
     * 查询在线商品列表
     */
    public static final String ListingItemList = "GetSellerList";
    /**

     *获取订单
     **/
    public static final String GetOrders = "GetOrders";
    /**
     * 获取订单商品
     */
    public static final  String GetItem="GetItem";
    /**
     * 卖家发送消息(订单中)
     */
    public static final  String AddMemberMessageAAQToPartner="AddMemberMessageAAQToPartner";
    /*
     *外部交易
     */
    public static final  String GetSellerTransactions="GetSellerTransactions";
    /*
     *GetAccount
     */
    public static final  String GetAccount="GetAccount";
    /*
     *评价及修改状态
     */
    public static final  String CompleteSale="CompleteSale";

    /*
     *一般的纠纷
     */
    public static final  String GetDispute="GetDispute";
    /**
     * 商品相似目录列表
     */
    public static final  String GetSuggestedCategories="GetSuggestedCategories";
    /**
     * 根据订单号同步订单
     */
    public static final  String GetOrderTransactions="GetOrderTransactions";

    /**
     * 检查账号状态
     */
    public static final  String CheckEbayTokenStatus="GetTokenStatus";

    /**
     * 设置ebay涨好的消息通知
     * */
    public static final String SetNotificationPreferences="SetNotificationPreferences";
    /**
     * 设置促销规则
     */
    public static final String SetPromotionalSale="SetPromotionalSale";
    /**
     * 设置商品到促销规则
     */
    public static final String SetPromotionalSaleListings = "SetPromotionalSaleListings";
    /**
     * 获取促销规则
     */
    public static final String GetPromotionalSaleDetails = "GetPromotionalSaleDetails";
    /**
     * 重复刊登，主要用于拍买结束重新上架
     */

    public static final String RelistItem = "RelistItem";
}

