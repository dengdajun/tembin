...
public class SiteMessageServiceImpl implements SiteMessageService {
    ...
    @Override
    public PublicSitemessage selectPublicSitemessageById(Long id) {
        PublicSitemessageExample example=new PublicSitemessageExample();
        PublicSitemessageExample.Criteria cr=example.createCriteria();
        cr.andIdEqualTo(id);
        List<PublicSitemessage> list=publicSitemessageMapper.selectByExample(example);
    ...
    @Override
    public List<CustomPublicSitemessage> querySiteMessage1(PublicSitemessage sitemessage, Page page) {
        SessionVO sessionVO = SessionCacheSupport.getSessionVO();
        Map map=new HashMap();
        map.put("messageTo",sessionVO.getId());
        map.put("reciver",sessionVO.getLoginId());
        if(!"all".equalsIgnoreCase(sitemessage.getMessageType())){
            map.put("messageType",sitemessage.getMessageType());
        }
        if(StringUtils.isNotEmpty(sitemessage.getReaded()) ){
        ...
        List<CustomPublicSitemessage> customPublicSitemessages = customPublicSitemessageMapper.selectSiteMessageList1(map, page);
        return customPublicSitemessages;
    }
    @Override
    public List<CustomPublicSitemessage> selectPublicSitemessageByMessageId(Map map,Page page) {
        return customPublicSitemessageMapper.selectPublicSitemessageByMessageId(map,page);
    }
}
