...
public class TradingFeedBackDetailImpl implements com.trading.service.ITradingFeedBackDetail {
    ...
    @Override
    public void saveTradingFeedBackDetailMy(TradingFeedBackDetail tradingFeedBackDetail){
        if(tradingFeedBackDetail.getId()!=null){
            this.tradingFeedBackDetailMapper.updateByPrimaryKeyWithBLOBs(tradingFeedBackDetail);
        }else{
            this.tradingFeedBackDetailMapper.insertSelective(tradingFeedBackDetail);
        }
    }
    ...
    /**
     *map.put("ebayAccount",ebay账号名称)
     * map.put("commentType",评价类型：Positive、Neutral等)，如需要询该账号所有评价就不该参数
     * @param map
     * @return
     */
    @Override
    public FeedBackReportQuery selectFeedBackEbayAccountReportList(Map map){
        List<FeedBackReportQuery> ls=feedBackReportMapper.selectFeedBackEbayAccountReportList(map);
    ...
    @Override
    public TradingFeedBackDetail selectByFeedBackId(long id){
        return this.tradingFeedBackDetailMapper.selectByPrimaryKey(id);
    }
}
