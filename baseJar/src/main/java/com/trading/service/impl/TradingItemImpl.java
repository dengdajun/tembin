...
public class TradingItemImpl implements com.trading.service.ITradingItem {
    ...
    public void saveTradingItem(TradingItemWithBLOBs pojo) throws Exception {
        if(pojo.getId()==null){
            ...
            pojo.setApplicationdata(UUIDUtil.getUUID());
            this.tradingItemMapper.insertSelective(pojo);
        }else{
            if(ObjectUtils.isLogicalNull(pojo.getApplicationdata())){
                pojo.setApplicationdata(UUIDUtil.getUUID());
            }
    ...
    public Map saveItem(Item item, TradingItemWithBLOBs tradingItem1) throws Exception {
        ...
        if(item.getVariations()!=null){
            if(item.getVariations().getVariationSpecificsSet()==null||item.getVariations().getVariationSpecificsSet().getNameValueList()==null||item.getVariations().getVariationSpecificsSet().getNameValueList().get(0)==null){
                Asserts.assertTrue(false,"多属性中主属性名称不能为空！");
            }
        ...
        for(int is =0;is<paypals.length;is++) {
            ...
            tradingItem.setCreateUser(c.getId());
            tradingItem.setEbayAccount(paypals[is]);
            if(is>=1){
                tradingItem.setId(null);
            }
            tradingItem.setTitle(HtmlUtils.htmlEscape(request.getParameter("Title_"+paypals[is])));
            ...
            UsercontrollerEbayAccount ueas = this.iUsercontrollerEbayAccount.selectById(Long.parseLong(paypals[is]));
            Long paypalAccountId = ueas.getPaypalAccountId();
            if(paypalAccountId==null||paypalAccountId==0){
                Asserts.assertTrue(false,ueas.getEbayName()+"未绑定paypal账号，不允许保存范本！");
            }else{
                TradingPaypal tpss = this.iTradingPayPal.selectBySitePaypalId(tradingItem.getSite(),ueas.getPaypalAccountId()+"");
                if(tpss!=null){
                    tradingItem.setPayId(tpss.getId());
                }
            }
            TradingItemWithBLOBs tradingItem_1 = this.selectByTitleSkuEbayAccount(tradingItem.getSku(),tradingItem.getTitle(),paypals[is],tradingItem.getListingtype(),tradingItem.getSite());
            if(tradingItem_1!=null&&paypals.length>1){
                tradingItem.setId(tradingItem_1.getId());
            }
    ...
    @Override
    public Item toItemByTradingItem(TradingItemWithBLOBs tradingItem) throws Exception{
        Item item = new Item();
        ...
        if(treturn!=null) {
            ...
            if(item.getReturnPolicy().getReturnsAcceptedOption()!=null) {
            ...
            if(item.getReturnPolicy().getReturnsWithinOption()!=null) {
            ...
            if(item.getReturnPolicy().getRefundOption()!=null) {
            ...
            if(item.getReturnPolicy().getShippingCostPaidByOption()!=null) {
        ...
        item.setDescription(tradingItem.getDescription());
        ...
        if(tp!=null&&!ObjectUtils.isLogicalNull(tp.getPaypal())&&!"null".equals(tp.getPaypal())) {
    ...
    public Item toItem(TradingItem tradingItem) throws Exception {
        Item item = new Item();
        ConvertPOJOUtil.convert(item,tradingItem);
        item.setTitle(HtmlUtils.htmlUnescape(tradingItem.getTitle()));
        if(tradingItem.getListingtype().equals("FixedPriceItem")){//固价刊登
            StartPrice sp = new StartPrice();
            sp.setCurrencyID(tradingItem.getCurrency());
            sp.setValue(tradingItem.getStartprice());
        ...
        item.setApplicationData(tradingItem.getUuid());
        if(tradingItem!=null) {
            Long storeCategoryId = tradingItem.getStoreCategoryId();
            if (!ObjectUtils.isLogicalNull(storeCategoryId)) {
                TradingStoreCategory tsc = this.iTradingStoreCategory.selectByEbayId(storeCategoryId);
                if(tsc!=null) {
                    Storefront sf = new Storefront();
                    sf.setStoreCategoryID(Long.parseLong(tsc.getStoreCategoryId()));
                    sf.setStoreCategoryName(tsc.getStoreCategoryName());
        ...
        if(tradingItem.getTemplateId()!=null){//如果选择了模板
            ...
            template = docs.html();
            Asserts.assertTrue(template!=null && item.getDescription()!=null,"商品描述不能为空！");
        ...
        if(litpd!=null&&litpd.size()>0){
            ...
            for(int i=0;i<litam.size();i++){
                TradingAttrMores tam = litam.get(i);
                Asserts.assertTrue(tam.getAttr1()!=null&&!"".equals(tam.getAttr1()),"第"+(i+1)+"张图片未成功上传，请重新选择图片上传");
                List<TradingListingpicUrl> liplu = this.iTradingListingPicUrl.selectByMackId(tam.getAttr1());
                if(liplu!=null&&liplu.size()>0){
                    TradingListingpicUrl plu = liplu.get(0);
                    String url = plu.getUrl();
    ...
    @Override
    public TradingItemWithBLOBs selectByTitleSkuEbayAccount(String sku,String title,String ebayAccount,String listingType,String site){
        TradingItemExample tie = new TradingItemExample();
        TradingItemExample.Criteria tc = tie.createCriteria();
        tc.andTitleEqualTo(title).andEbayAccountEqualTo(ebayAccount).andListingtypeEqualTo(listingType).andSiteEqualTo(site).andCheckFlagEqualTo("0");
        if(sku!=null){
            tc.andSkuEqualTo(sku);
        }
        List<TradingItemWithBLOBs> liti = this.tradingItemMapper.selectByExampleWithBLOBs(tie);
        if(liti==null||liti.size()==0){
            return null;
    ...
    @Override
    public TradingItemWithBLOBs selectApplitionData(String appstr){
        TradingItemExample tie = new TradingItemExample();
        TradingItemExample.Criteria tc = tie.createCriteria();
        tc.andApplicationdataEqualTo(appstr);
        List<TradingItemWithBLOBs> liti = this.tradingItemMapper.selectByExampleWithBLOBs(tie);
        if(liti==null||liti.size()==0){
            return null;
        }else{
    ...
    @Override
    public void updateTradingItem(Item item, TradingItemWithBLOBs tradingItem,String id) throws Exception {
        HttpServletRequest request = RequestResponseContext.getRequest();
        ...
        TradingItem tradingItem1=this.selectByIdBL(Long.parseLong(id));
        for(String str : selectType){
            if(str.equals("StartPrice")&&item.getStartPrice()!=null){//改价格
                ...
                if(tp!=null) {
                    ...
                    upa.createCriteria().andEmailEqualTo(item.getPayPalEmailAddress()).andStatusEqualTo("1");
                    List<UsercontrollerPaypalAccount> li = this.usercontrollerPaypalAccountMapper.selectByExample(upa);
                    if(li!=null&&li.size()>0){
                        tp.setPaypal(li.get(0).getId()+"");
                        tp.setPayName(li.get(0).getPaypalAccount());
                ...
                if(addr!=null){
                    ...
                    for(TradingDataDictionary pdd : li ){
                        if(pdd.getValue().equals(item.getCountry())){
                            pd = pdd;
                            break;
                        }
                    }
                ...
                tradingItem.setPrivatelisting(item.getPrivateListing()==null?"1":"0");
        ...
        if(selectType!=null&&selectType.length>0){
            tradingItem.setUuid(tradingItem1.getUuid());
    ...
    public void saveListingItem(Item item, KeyMoveList kml,String categoryName) throws Exception {
        //for(int is = 0;is < liitem.size();is++){
            ...
            if(liti==null||liti.size()==0) {
                ...
                tradingItem.setItemName(StringEscapeUtils.escapeHtml(item.getTitle()));
                ...
                tradingItem.setSku(item.getSKU());
                tradingItem.setTitle(StringEscapeUtils.escapeHtml(item.getTitle()));
...
*/
                ...
                if (picd != null) {
                    ...
                    //下载范本列表中的小图片
                    if(!ObjectUtils.isLogicalNull(tpicd.getGalleryurl())){
                ...
                //运输选项
                this.saveShipping(tradingItem,item,kml);
                this.tradingItemMapper.updateByPrimaryKeySelective(tradingItem);
            }else{
                //单独处理运输选项，如果数据已保存，那么将更新操作，等数据跑完后，该代码可注释掉
                TradingItemWithBLOBs tradingItem = liti.get(0);
                this.saveShipping(tradingItem,item,kml);
                this.tradingItemMapper.updateByPrimaryKeySelective(tradingItem);
            }
   ...
   // }
    ...
    private void saveShipping(TradingItemWithBLOBs tradingItem,Item item,KeyMoveList kml){
        try{
            //运输选项
            if (item.getShippingDetails() != null) {
                ...
                List<TradingShippingdetails> lits = .selectByExample(tse);*/
                //得到判断条件
                List<String> listr = new ArrayList<>();
                List<ShippingServiceOptions> liissso = shippingDetails.getShippingServiceOptions();
                if(liissso!=null&&liissso.size()>0){
                    for(ShippingServiceOptions issso:liissso){
                        listr.add(issso.getShippingService());
                    }
                }
                List<InternationalShippingServiceOption> liisisso = shippingDetails.getInternationalShippingServiceOption();
                if(liisisso!=null&&liisisso.size()>0){
                    for(InternationalShippingServiceOption isisso:liisisso){
                        listr.add(isisso.getShippingService());
                    }
                }
                int orgId = this.usercontrollerUserMapper.selectByPrimaryKey(tradingItem.getCreateUser().intValue()).getUserOrgId();
                String site = tradingItem.getSite();
                List<String> liabc = new ArrayList();
                if(listr!=null&&listr.size()>0){
                    DataDicShipingQueryParamVO ddsqpv = new DataDicShipingQueryParamVO();
                    ddsqpv.setOrgID(Long.parseLong(orgId+""));
                    ddsqpv.setSiteID(Long.parseLong(site));
                    ddsqpv.setShipingString(listr);
                    liabc = DataDictionarySupport.checkShipingIsExit(ddsqpv);
                }
                if (liabc==null||liabc.size()==0) {
                    ...
                    for (int i = 0; i < lisso.size(); i++) {
                        ...
                        //保存国内运输选项费用信息
                        TradingShippingserviceoptionsDoc tsds = new TradingShippingserviceoptionsDoc();
                        ConvertPOJOUtil.convert(tsds,tsp);
                        if(tsds.getShippingservice()==null){
                            continue;
                        }
                        tsds.setShippingservicecost(tsp.getShippingservicecost());
                        tsds.setShippingserviceadditionalcost(tsp.getShippingserviceadditionalcost());
                        tsds.setShippingsurcharge(tsp.getShippingsurcharge());
                        tsds.setSourceId(tsp.getId());
                        tsds.setParentId(tradingItem.getShippingDeailsId());
                        tsds.setParentUuid(tradingItem.getUuid());
                        tsds.setDocId(tradingItem.getId());
                        tsds.setCreateUser(tsp.getCreateUser());
                        tsds.setCreateTime(new Date());
                        litsds.add(tsds);
                    }
                    ...
                    if (shippingDetails.getInternationalShippingServiceOption() != null) {
                        ...
                        for (int i = 0; i < liinter.size(); i++) {
                            ...
                            if(litdd!=null&&litdd.size()>0) {
                                ...
                                TradingDataDictionary tdds = new TradingDataDictionary();
                                tdds.setName(tiss.getShippingservice());
                                tdds.setValue(tiss.getShippingservice());
                                tdds.setType("International transport");
                                tdds.setValue1("2");
                                tdds.setName1("Other services");
                                tdds.setDataDesc("国际运输");
                                this.iTradingDataDictionary.saveDataDictionary(tdds);
                                tiss.setShippingservice(tdds.getId()+"");
                            }
                            ...
                            TradingInternationalshippingserviceoptionDoc tidoc = new TradingInternationalshippingserviceoptionDoc();
                            ConvertPOJOUtil.convert(tidoc,tiss);
                            if(tidoc.getShippingservice()==null){
                                continue;
                            }
                            tidoc.setShippingservicecost(tiss.getShippingservicecost());
                            tidoc.setShippingserviceadditionalcost(tiss.getShippingserviceadditionalcost());
                            tidoc.setSourceId(tiss.getId());
                            tidoc.setParentId(tradingItem.getShippingDeailsId());
                            tidoc.setParentUuid(tradingItem.getUuid());
                            tidoc.setDocId(tradingItem.getId());
                            tidoc.setCreateUser(tiss.getCreateUser());
                            tidoc.setCreateTime(new Date());
                            liidoc.add(tidoc);
                    ...
                    TradingShippingdetails ts = this.tradingShippingdetailsMapper.selectByPrimaryKey(Long.parseLong(liabc.get(0)));
                    tradingItem.setShippingDeailsId(ts.getId());
                    //保存费用信息
                    List<TradingShippingserviceoptions> litss = this.iTradingShippingServiceOptions.selectByParentId(ts.getId());
                    if(litss!=null&&litss.size()>0){
                        List<TradingShippingserviceoptionsDoc> litsds = new ArrayList<TradingShippingserviceoptionsDoc>();
                        for(TradingShippingserviceoptions tsp:litss){
                            TradingShippingserviceoptionsDoc tsds = new TradingShippingserviceoptionsDoc();
                            ConvertPOJOUtil.convert(tsds,tsp);
                            if(tsds.getShippingservice()==null){
                                continue;
                    ...
                    List<TradingInternationalshippingserviceoption> litis = this.iTradingInternationalShippingServiceOption.selectByParentid(ts.getId());
                    if(litis!=null&&litis.size()>0){
                        List<TradingInternationalshippingserviceoptionDoc> liidoc = new ArrayList<TradingInternationalshippingserviceoptionDoc>();
                        for(TradingInternationalshippingserviceoption tiss:litis){
                            TradingInternationalshippingserviceoptionDoc tidoc = new TradingInternationalshippingserviceoptionDoc();
                            ConvertPOJOUtil.convert(tidoc,tiss);
                            if(tidoc.getShippingservice()==null){
                                continue;
    ...
    public void copyItem(String[] ids,String ebayaccount) throws Exception {
        for(String id:ids){
            ...
            tradingItem.setUuid(UUIDUtil.getUUID());
            this.saveTradingItem(tradingItem);
            //复制模板图片
            List<TradingAttrMores> litamt = this.iTradingAttrMores.selectByParnetid(oldid,"TemplatePicUrl");
            if(litamt!=null&&litamt.size()>0){
                for(TradingAttrMores tamt:litamt){
                    TradingAttrMores tam = this.iTradingAttrMores.toDAOPojo("TemplatePicUrl",tamt.getValue());
                    tam.setParentId(tradingItem.getId());
                    tam.setParentUuid(tradingItem.getUuid());
                    this.iTradingAttrMores.saveAttrMores(tam);
                }
    ...
    @Override
    public TradingItem selectItemBySkuAndUserId(String sku,List<UsercontrollerUserExtend> orgUsers) {
        TradingItemExample example=new TradingItemExample();
        TradingItemExample.Criteria cr=example.createCriteria();
        cr.andSkuEqualTo(sku);
        if(orgUsers!=null&&orgUsers.size()>0){
            List<Long> userIds=new ArrayList<>();
            for(UsercontrollerUserExtend orgUser:orgUsers){
                userIds.add(Long.valueOf(orgUser.getUserId()));
            }
            cr.andCreateUserIn(userIds);
            List<TradingItem> list=tradingItemMapper.selectByExample(example);
    ...
    @Override
    public List<TradingItemWithBLOBs> selectByEbayAccount(String ebayAccount){
        TradingItemExample tie = new TradingItemExample();
        tie.createCriteria().andEbayAccountEqualTo(ebayAccount).andCheckFlagEqualTo("0");
        return this.tradingItemMapper.selectByExampleWithBLOBs(tie);
    }

    @Override
    public List<ItemQuery> selectByItemIdList(){
        return this.itemMapper.selectByItemIdList();
    }
}
