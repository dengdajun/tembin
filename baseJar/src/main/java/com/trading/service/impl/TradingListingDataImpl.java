...
public class TradingListingDataImpl implements com.trading.service.ITradingListingData {
    ...
    @Override
    public List<ListingDataQuery> selectByEbayAccountSite(Map map,Page page){
        return this.listingDataMapper.selectByEbayAccountSite(map,page);
    }


    @Override
    public List<ListingDataQuery> selectByonSite(Map map,Page page){
        return this.listingDataMapper.selectByonSite(map,page);
    }
    ...
    public List<TradingListingData> selectByList(String sku,String ebayAccount){
        ...
        tlde.createCriteria().andSkuEqualTo(sku).andEbayAccountEqualTo(ebayAccount).andIsFlagEqualTo("0");
    ...
    @Override
    public TradingListingData selectBySkuAndEbayAcount(String sku, String ebayAcount) {
        TradingListingDataExample example=new TradingListingDataExample();
        TradingListingDataExample.Criteria cr=example.createCriteria();
        cr.andSkuEqualTo(sku);
        cr.andEbayAccountEqualTo(ebayAcount);
        cr.andIsFlagEqualTo("0");
        List<TradingListingData> list=tradingListingDataMapper.selectByExample(example);
        return list!=null&&list.size()>0?list.get(0):null;
    }

    @Override
    public List<ListingDataQuery> selectListDateByTwentySku(Map map, Page page) {
        return listingDataMapper.selectListDateByTwentySku(map,page);
    }
}
