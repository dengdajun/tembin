...
public class ScheduleGetTimerOrdersImpl implements IScheduleGetTimerOrders {
    ...
    public void synchronizeOrders(List<TaskGetOrders> taskGetOrders) throws Exception {
        ...
        for(TaskGetOrders taskGetOrder:taskGetOrders){
                ...
                d.setRepetitionMark("getOrders_" + taskGetOrder.getId());
            //真实
            ...
            if (StringUtils.isBlank(taskGetOrder.getToken())){
                UsercontrollerEbayAccount account = ebayAccountMapper.selectByPrimaryKey(taskGetOrder.getEbayid());
                taskGetOrder.setToken(account.getEbayToken());
            }
            ...
            if(StringUtils.isBlank(res)){
                ...
                if ("fail".equalsIgnoreCase(r1)) {
                    ...
                    logger.error("定时同步订单调用API失败:(错误码:"+errorCode+")"+res);
                    continue;
                }
                String ack="";
                try{
            ...
            //-----------------------------
                if(!"Success".equalsIgnoreCase(ack)&& !"Warning".equalsIgnoreCase(ack)){
                    ...
                    for(String messageA:EbayErrorCodeStatic.orderArray){
                        if(StringUtils.isNotBlank(messageA)&&messageA.equals(errorCode11)){
                            TempStoreDataSupport.removeData("ApiFail_" + APINameStatic.GetOrders);
                        }
                    }
                }
            //-----------------------------
                if (("Success".equalsIgnoreCase(ack)||"Warning".equalsIgnoreCase(ack))&&res!=null) {
                    if("Warning".equalsIgnoreCase(ack)){
                       ...
                       /* if(list1==null||list1.size()==0){
                        ...
                        String errorCode=SamplePaseXml.getErrorCode(res);
                        if (EbayErrorCodeStatic.associated_Items_been_delete.equals(errorCode)){
                            //nothing todo
                        }else {
                            logger.error("获取定时订单有警告!(错误码:"+errorCode+")" + errors);
                        }
                   ...
                   /* resMap = addApiTask.exec2(d, xml, "https://api.ebay.com/ws/api.dll");*/
                            ...
                            if ("apiFail".equalsIgnoreCase(res)){return;}
                            if ("fail".equalsIgnoreCase(r1)) {
                                String errorCode=SamplePaseXml.getErrorCode(res);
                                logger.error("循环中的定时同步订单调用API失败(错误码:"+errorCode+"):"+res);
                                continue;
                            }
                            ack="";
                            try{
                            ...
                            if (!"Success".equalsIgnoreCase(ack)&&!"Warning".equalsIgnoreCase(ack)) {
                                ...
                                if (errors==null){
                                    errors=MyStringUtil.getStringBetween2char(res,"<Errors>","</Errors>");
                                }
                                String errorCode=SamplePaseXml.getErrorCode(res);
                                logger.error("循环中的Order参数错误!(错误码:"+errorCode+")" + errors+"\n\nXML:");
                            }
                            if("Warning".equalsIgnoreCase(ack)){
                                String errors = "";
                                try{
                                ...
                                if (EbayErrorCodeStatic.associated_Items_been_delete.equals(errorCode)){
                                    ...
                                    logger.error("循环中的获取定时订单有警告!(错误码:"+errorCode+")" + errors);
                                }
                        ...
                        if (("Success".equalsIgnoreCase(ack)||"Warning".equalsIgnoreCase(ack))&&res!=null) {
                            ...
                            for (TradingOrderGetOrders order : orders) {
                                List<TradingOrderGetOrders> ls = iTradingOrderGetOrders.selectOrderGetOrdersByTransactionId(order.getTransactionid(),taskGetOrder.getEbayid(),order.getItemid());
                                if(ls!=null&&ls.size()>0){
                                    order.setId(ls.get(0).getId());
                                    order.setCreateTime(ls.get(0).getCreateTime());
                                    order.setUuid(ls.get(0).getUuid());
                                    order.setPaypalflag(order.getPaypalflag());
                                    order.setShippedflag(order.getShippedflag());
                                    if(StringUtils.isNotBlank(ls.get(0).getComment())){
                                        order.setComment(ls.get(0).getComment());
                                    }
                                    ...
                                    if(StringUtils.isNotBlank(ls.get(0).getBuyerpayalacount())){
                                        order.setBuyerpayalacount(ls.get(0).getBuyerpayalacount());
                                    }
                                    order.setPaypalcountryname(ls.get(0).getPaypalcountryname());
                                    order.setPaypalname(ls.get(0).getPaypalname());
                                    order.setPaypalpostalcode(ls.get(0).getPaypalpostalcode());
                                    order.setPaypalstreet1(ls.get(0).getPaypalstreet1());
                                    order.setPaypalstreet2(ls.get(0).getPaypalstreet2());
                                    order.setPaypalcityname(ls.get(0).getPaypalcityname());
                                    Date canceltime=ls.get(0).getCanceltime();
                                    if(canceltime==null){
                                        String cancelstatus=order.getCancelstatus();
                                        if(StringUtils.isNotBlank(cancelstatus)&&(cancelstatus.contains("CancelClosed")||cancelstatus.equals("CancelComplete"))){
                                             order.setCanceltime(order.getLastmodifiedtime());
                                        }
                                    }else{
                                ...
                                Date date12=DateUtils.buildDateTime(2015,1,1,3,0,0);
                                Date date2=DateUtils.turnToDateStart(date12);
                                if (addmessages != null && addmessages.size() > 0) {
                                   // boolean flag = false;
                                    String trackingnumber = order.getShipmenttrackingnumber();
                                    Date shipedTime = order.getShippedtime();
                                    Date modifyDate=order.getLastmodifiedtime();
                                    List<TradingAutoMessageAndOrder> autoMessageAndOrders=new ArrayList<>();
                                    if(order.getId()!=null){
                                        autoMessageAndOrders=iTradingAutoMessageAndOrder.selectAutoMessageAndOrderByAutoOrderId(order.getId());
                                    }
                                    //order=autoMessageSet("shippedAutoMessage",taskGetOrder.getUserid(),order);
                                    if(autoMessageAndOrders!=null&&autoMessageAndOrders.size()>0){
                                        for(TradingAutoMessageAndOrder autoMessageAndOrder:autoMessageAndOrders){
                                            List<TradingOrderAddMemberMessageAAQToPartner> partners=iTradingOrderAddMemberMessageAAQToPartner.selectTradingOrderAddMemberMessageAAQToPartnerByAutoMessageIdAndOrderId(autoMessageAndOrder.getAutomessageId(),autoMessageAndOrder.getOrderId(),"true");
                                            if((partners==null||partners.size()==0)&& shipedTime!=null &&modifyDate.after(date2)){
                                                Date shiptime=order.getShippedtime();
                                                Date shiptime1=DateUtils.addDays(shiptime,2);
                                                Date nowDate=new Date();
                                                if(nowDate.before(shiptime1)){
                                        ...
                                        if(order.getId()!=null){
                                            List<TradingOrderAddMemberMessageAAQToPartner> partners=iTradingOrderAddMemberMessageAAQToPartner.selectTradingOrderAddMemberMessageAAQToPartnerByOrderIdAndMessageTypeAndMessageFlag(order.getId(),2,2,"true");
                                            if((partners==null||partners.size()==0)&&shipedTime!=null&&modifyDate.after(date2)){
                                                Date shiptime=order.getShippedtime();
                                                Date shiptime1=DateUtils.addDays(shiptime,2);
                                                Date nowDate=new Date();
                                    ...
                                    String trackingnumber = order.getShipmenttrackingnumber();
                                    Date shipedTime = order.getShippedtime();
                                    if ("Complete".equals(order.getStatus()) && shipedTime==null && "Completed".equals(order.getOrderstatus())) {
                                        //付款后发送消息
                                        //order=autoMessageSet("paidAutoMessage",taskGetOrder.getUserid(),order);
                                        //修改6.1
                                        List<TradingOrderAddMemberMessageAAQToPartner> partners=new ArrayList<>();
                                        if(order.getId()!=null){
                                            partners=iTradingOrderAddMemberMessageAAQToPartner.selectTradingOrderAddMemberMessageAAQToPartnerByOrderIdAndMessageTypeAndMessageFlag(order.getId(),2,1,"false");
                                        }
                                        //修改6.1
                                        Date creatDate=order.getCreatedtime();
                                        if(creatDate.after(date1)&&(partners==null||partners.size()==0)){
                                            order.setPaypalflag(1);
                                            order.setShippedflag(0);
                                            order.setAutomessageId(1L);
                                        }else{
                                    ...
                                    }else if ("Complete".equals(order.getStatus())&&shipedTime!=null&& "Completed".equals(order.getOrderstatus())) {
                                        //order=autoMessageSet("shippedAutoMessage",taskGetOrder.getUserid(),order);
                                        Date modifyDate=order.getLastmodifiedtime();
                                ...
                                order.setEbayId(taskGetOrder.getEbayid());
                    ...
                    String errorCode=SamplePaseXml.getErrorCode(res);
                    logger.error("Order更新失败!(错误码:"+errorCode+")"+LongError+":resxml"+res);
                }
                if(taskGetOrder.getNewuserflag()==null){
                    Integer flag1=taskGetOrder.getTokenflag();
                    flag1=flag1+1;
                    taskGetOrder.setTokenflag(flag1);
                    taskGetOrder.setLastsyctime(new Date());
   ...
   /* private Date sendAutoMessageTime(TradingAutoMessage partner,TradingOrderGetOrders order){
    ...
    public void synchronizeOrderItems(List<TradingOrderGetOrders> orders) throws Exception {
       ...
       /* UsercontrollerDevAccountExtend d=new UsercontrollerDevAccountExtend();
        ...
        for(TradingOrderGetOrders order:orders){
            ...
            if (order==null||StringUtils.isBlank(order.getItemid())){
                continue;
            }
            ...
            if(ebay!=null&&StringUtils.isNotBlank(ebay.getEbayToken())) {
                ...
                if ("fail".equalsIgnoreCase(itemr1)) {
                    String errorCode=SamplePaseXml.getErrorCode(itemres);
                    logger.error("定时同步订单商品调用API失败(错误码:"+errorCode+"):" +itemres);
                    continue;
                }
                String itemack = "";
                try {
                ...
                //------------------------------------------------------
                if(!"Success".equalsIgnoreCase(itemack)&& !"Warning".equalsIgnoreCase(itemack)){
                    String errorCode11=SamplePaseXml.getErrorCode(itemres);
                    for(String messageA:EbayErrorCodeStatic.orderItemArray){
                        if(StringUtils.isNotBlank(messageA)&&messageA.equals(errorCode11)){
                            TempStoreDataSupport.removeData("ApiFail_"+APINameStatic.GetItem);
                        }
                    }
                }
                //------------------------------------------------------
                if ("Success".equalsIgnoreCase(itemack)&&itemres!=null) {
                ...
                }else if(itemres==null){
                    continue;
                } else {
                    String LongError=SamplePaseXml.getWarningInformation(itemres);
                    String errorCode=SamplePaseXml.getErrorCode(itemres);
                    String xml=itemresmap.get("xml");
                    logger.error("Order定时同步商品获取apisessionid失败!(错误码:"+errorCode+"):>>xml:"+xml+">>res:" +itemres);
                }
            }
            order.setItemflag(1);
            TaskPool.togos.put(order);
       ...
       /* UsercontrollerDevAccountExtend ds=new UsercontrollerDevAccountExtend();
        ...
        for(TradingOrderGetOrders order:orders){
            ...
            if(ebay!=null&&StringUtils.isNotBlank(ebay.getEbayToken())) {
                ...
                if ("apiFail".equalsIgnoreCase(accountres)){return;}
                if ("fail".equalsIgnoreCase(accountr1)) {
                    String errorCode=SamplePaseXml.getErrorCode(accountres);
                    logger.error("定时同步订单Account调用API失败(错误码:"+errorCode+"):"+accountres);
                    continue;
                }
                String accountack="";
                try{
                ...
                //-------------
                if(!"Success".equalsIgnoreCase(accountack)&& !"Warning".equalsIgnoreCase(accountack)){
                    String errorCode11=SamplePaseXml.getErrorCode(accountres);
                    for(String messageA:EbayErrorCodeStatic.orderAcountArray){
                        if(StringUtils.isNotBlank(messageA)&&messageA.equals(errorCode11)){
                            TempStoreDataSupport.removeData("ApiFail_"+APINameStatic.GetAccount);
                        }
                    }
                }
                //------------
                if ("Success".equalsIgnoreCase(accountack)&&accountres!=null) {
                    ...
                    String errorCode=SamplePaseXml.getErrorCode(accountres);
                    logger.error("Order定时同步account获取apisessionid失败!(错误码:"+errorCode+")" +accountres);
                }
            }
    ...
    public void synchronizeOrderSellerTrasaction(TaskGetOrdersSellerTransaction sellerTransaction) throws Exception {
       ...
       /* UsercontrollerDevAccountExtend d=new UsercontrollerDevAccountExtend();
        ...
        String start= sellerTransaction.getFromtime();
        String end=sellerTransaction.getTotime();
        ...
        if ("fail".equalsIgnoreCase(sellerR1)) {
            String errorCode=SamplePaseXml.getErrorCode(sellerRes);
            logger.error("定时同步订单外部交易的API失败(错误码:"+errorCode+"):"+sellerRes);
            return;
        }
        String sellerAck="";
        try{
        ...
        //------------------------
        if(!"Success".equalsIgnoreCase(sellerAck)&& !"Warning".equalsIgnoreCase(sellerAck)){
            String errorCode11=SamplePaseXml.getErrorCode(sellerRes);
            for(String messageA:EbayErrorCodeStatic.orderSellerTransactionArray){
                if(StringUtils.isNotBlank(messageA)&&messageA.equals(errorCode11)){
                    TempStoreDataSupport.removeData("ApiFail_"+"GetSellerTransactions");
                }
            }
        }
        //------------------------
        if ("Success".equalsIgnoreCase(sellerAck)&&sellerRes!=null) {
            ...
            for(int i=0;i<total;i++){
                ...
                if("apiFail".equalsIgnoreCase(sellerRes)){return;}
                if ("fail".equalsIgnoreCase(sellerR1)) {
                    String errorCode=SamplePaseXml.getErrorCode(sellerRes);
                    logger.error("定时同步订单外部交易的API失败799(错误码:"+errorCode+"):"+sellerRes);
                    continue;
                }
                sellerAck="";
                try{
                ...
                if ("Success".equalsIgnoreCase(sellerAck)&&sellerRes!=null) {
                    ...
                    for (TradingOrderGetSellerTransactions list : lists) {
                        List<TradingOrderGetSellerTransactions> transactionseList = iTradingOrderGetSellerTransactions.selectTradingOrderGetSellerTransactionsByTransactionIdandItemId(list.getItemid(),sellerTransaction.getEbayId(),list.getTransactionid());
            ...
            if (StringUtils.isBlank(errorCode)){
                logger.error("Order定时同步外部交易获取apisessionid失败!" );
            }else {
                logger.error("Order定时同步外部交易获取apisessionid失败!(错误码:"+errorCode+")" +LongError);
            }
