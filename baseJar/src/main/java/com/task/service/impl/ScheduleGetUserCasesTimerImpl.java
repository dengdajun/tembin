        ...
        /*CommAutowiredClass commPars = (CommAutowiredClass) ApplicationContextUtil.getBean(CommAutowiredClass.class);*/
            ...
            /*    d.setApiDevName("bbafa7e7-2f98-4783-9c34-f403faeb007f");
                ...
                if ("fail".equalsIgnoreCase(r1)) {
                    String errorId=SamplePaseXml.getCaseerrorId(res);
                    logger.error("定时纠纷调用API失败(错误码:"+errorId+")"+res+"\n\nXML:"+xml);
                    continue;
                }
                String ack = "";
                try{
                ...
                if(!"Success".equalsIgnoreCase(ack)&& !"Warning".equalsIgnoreCase(ack)){
                    String errorCode11=SamplePaseXml.getCaseerrorId(res);
                    for(String messageA:EbayErrorCodeStatic.userCase){
                        if(StringUtils.isNotBlank(messageA)&&messageA.equals(errorCode11)){
                            TempStoreDataSupport.removeData("ApiFail_" + "getUserCases");
                        }
                    }
                }
                //------------------------
                if ("Success".equalsIgnoreCase(ack)||"Warning".equalsIgnoreCase(ack)) {
                    if("Warning".equalsIgnoreCase(ack)){
                        ...
                        if(StringUtils.isBlank(errors)){
                            try{
                                errors = SamplePaseXml.getCaseWarningInformation(res);
                            }catch (Exception e){
                       ...
                       /* if(list1==null||list1.size()==0){
                        ...
                        String errorId=SamplePaseXml.getCaseerrorId(res);
                        if(!"1302".equals(errorId)){
                            logger.error("定时同步纠纷有警告!(错误码:"+errorId+")" + errors);
                        }
                    ...
                    for(int i=1;i<=totalPage;i++){
                        if(i!=1){
                            ...
                            if ("fail".equalsIgnoreCase(r1)) {
                                String errorId=SamplePaseXml.getCaseerrorId(res);
                                logger.error("循环中的定时纠纷API调用失败(错误码:"+errorId+"):"+res+"\n\nXML:"+xml);
                                continue;
                            }
                            if (!"Success".equalsIgnoreCase(ack)&&!"Warning".equalsIgnoreCase(ack)){
                                String errorCode11=SamplePaseXml.getCaseerrorId(res);
                                ...
                                String errorId=SamplePaseXml.getCaseerrorId(res);
                                if(!EbayErrorCodeStatic.user_cases_io_code.equals(errorId)){
                                    errorId=SamplePaseXml.getCaseerrorId(res);
                                    logger.error("循环中的定时同步纠纷API参数错误(错误码:"+errorId+")"+res+"\n\nXML:"+xml);
                                }
                            }
                            if("Warning".equalsIgnoreCase(ack)){
                                String errors = "";
                                ...
                                String errorId=SamplePaseXml.getCaseerrorId(res);
                                logger.error("获取定时CASE有警告!(错误码:"+errorId+")" + errors);
                            }
                            try{
                                map1= UserCasesAPI.parseXMLAndSave(res);
                            }catch (Exception e){
                        ...
                        if ("Success".equalsIgnoreCase(ack)||"Warning".equalsIgnoreCase(ack)) {
                            ...
                            for (TradingGetUserCases userCases : userCasesList) {
                                List<TradingGetUserCases> casesList = iTradingGetUserCases.selectGetUserCasesByTransactionId(userCases.getTransactionid(), taskGetUserCases.getEbayId());
                                ...
                                userCases.setEbayId(taskGetUserCases.getEbayId());
                                ...
                                if (caseType.contains("EBP")) {
                                    ...
                                    if ("fail".equalsIgnoreCase(ebpR1)) {
                                        String caseid = "";
                                        try {
                                            caseid=MyStringUtil.getStringBetween2char(ebpXml, "<caseId>", "</caseId>");
                                        } catch (Exception e) {
                                            caseid=ebpXml;
                                        }
                                        String errorId=SamplePaseXml.getCaseerrorId(ebpRes);
                                        logger.error("定时同步CASE_EBP的API失败(错误码:"+errorId+")"+ebpRes+"\n\nXML:"+caseid);
                                        continue;
                                    }
                                    String ebpAck = "";
                                    try{
                                    ...
                                    if(!"Success".equalsIgnoreCase(ebpAck)&& !"Warning".equalsIgnoreCase(ebpAck)){
                                        String errorCode11=SamplePaseXml.getCaseerrorId(ebpRes);
                                        for(String messageA:EbayErrorCodeStatic.EBPCase){
                                            if(StringUtils.isNotBlank(messageA)&&messageA.equals(errorCode11)){
                                                TempStoreDataSupport.removeData("ApiFail_" + "getEBPCaseDetail");
                                            }
                                        }
                                    }
                                    if ("Success".equalsIgnoreCase(ebpAck)||"Warning".equalsIgnoreCase(ack)) {
                                        ...
                                        if(caseDetail!=null){
                                            List<TradingGetEBPCaseDetail> lists = iTradingGetEBPCaseDetail.selectGetEBPCaseDetailByTransactionId(taskGetUserCases.getEbayId(),caseDetail.getTransactionid());
                                            ...
                                            caseDetail.setSellerid(userCases.getSellerid());
                                            caseDetail.setEbayId(taskGetUserCases.getEbayId());
                                        ...
                                        String errorId=SamplePaseXml.getCaseerrorId(ebpRes);
                                        if(!EbayErrorCodeStatic.cases_ebp_io_code.equals(errorId)&&!"1502".equals(errorId)){
                                            String caseid = "";
                                            try {
                                                caseid=MyStringUtil.getStringBetween2char(ebpXml, "<caseId>", "</caseId>");
                                            } catch (Exception e) {
                                            ...
                                            errorId=SamplePaseXml.getCaseerrorId(ebpRes);
                                            logger.error("定时同步CASE_EBP的参数错误(错误码:"+errorId+")"+ebpRes+"\n\nXML:"+caseid);
                                        }
                                    }
                                } else {
                                    d = new UsercontrollerDevAccountExtend();
                               ...
                               /* d.setApiDevName("5d70d647-b1e2-4c7c-a034-b343d58ca425");
                                ...
                                d.setApiCallName(APINameStatic.GetDispute);*/
                                    ...
                                    r1 = resMap.get("stat");
                                    res = resMap.get("message");
                                    if ("apiFail".equalsIgnoreCase(res)){
                                        return;
                                    }
                                    if ("fail".equalsIgnoreCase(r1)) {
                                        String errorCode=SamplePaseXml.getErrorCode(res);
                                        logger.error("定时同步CASE_dispute的API失败(错误码:"+errorCode+")"+res+"\n\nXML:"+xml);
                                        continue;
                                    }
                                    String Ack = "";
                                    try{
                                    ...
                                    if(!"Success".equalsIgnoreCase(Ack)&& !"Warning".equalsIgnoreCase(Ack)){
                                        ...
                                        for(String messageA:EbayErrorCodeStatic.disputeCase){
                                            if(StringUtils.isNotBlank(messageA)&&messageA.equals(errorCode11)){
                                                TempStoreDataSupport.removeData("ApiFail_" + APINameStatic.GetDispute);
                                            }
                                        }
                                    }
                                    if ("Success".equalsIgnoreCase(Ack)) {
                                        ...
                                        try{
                                           ...
                                           dispute = (TradingGetDispute) disputeMap.get("dispute");
                                            ...
                                            logger.error("ScheduleGetUserCasesTimerImpl 解析DISPUTE XML出错281:"+res,e);
                                        }
                                        if(dispute!=null){
                                            List<TradingGetDispute> disputeList = iTradingGetDispute.selectGetDisputeByTransactionId(taskGetUserCases.getEbayId(),dispute.getTransactionid());
                                            ...
                                            dispute.setEbayId(taskGetUserCases.getEbayId());
                                            //iTradingGetDispute.saveGetDispute(dispute);
                                            TaskPool.togos1.put(dispute);
                                            if("0".equals(TaskPool.togosBS1[0])){
                                                iTradingOrderGetOrdersNoTransaction.saveGetDispute(null);
                                            }
                                            List<TradingGetDisputeMessage> messageList = (List<TradingGetDisputeMessage>) disputeMap.get("disputeMessage");
                                            List<TradingGetDisputeMessage> list =new ArrayList<>();
                                            if(dispute.getId()!=null){
                                                list= iTradingGetDisputeMessage.selectGetDisputeMessageByDisputeId(dispute.getId());
                                            }
                                        ...
                                        String errorCode=SamplePaseXml.getErrorCode(res);
                                        logger.error("定时同步CASE_dispute的参数错误(错误码:"+errorCode+")"+res+"\n\nXML:"+xml);
                                    }
                                }
                            }
                        }
                    ...
                    String errorId=SamplePaseXml.getCaseerrorId(res);
                    if(!EbayErrorCodeStatic.user_cases_io_code.equals(errorId)){
                        logger.error("定时同步纠纷API参数错误"+taskGetUserCases.getEbayname()+">>"+res+"\n\nXML:"+xml);
                    }
                }
                Integer flag=taskGetUserCases.getTokenflag();
                flag=flag+1;
                taskGetUserCases.setTokenflag(flag);
                taskGetUserCases.setLastsyctime(new Date());
