...
<head>
    ...
    <script type="text/javascript">
        ...
        function refreshTable3(amount,status,day,type,content,starttime,endtime,messageType){
            $("#MessageGetmymessageListTable2").selectDataAfterSetParm({"bedDetailVO.deptId":"", "isTrue":0,"amount":amount,"status":status,"day":day,"type":type,"content":content,"starttime1":starttime,"endtime1":endtime,"messageType":messageType});

        }
        /**查看消息*/
        var OrderGetOrders;
        function makeOption1(json,messageid,id,replied){
            var src1=path+"/img/";
            ...
            if(replied=="true"){
                ...
                if(json!=null&&json.replied=='true'){
                    tds[1].innerHTML= "<img src='"+src1+"replied.png' title='已回复'/>"
                }else{
        ...
        function makeOption8(json){
            ...
            var pp={"liString":hs};
            return "<div align='center' style='width: 100%'>"+getULSelect(pp)+"</div>";
        }
        function selectOption(messageid,id,obj,replied){
            var value=$(obj).attr("value");
            if(value=='1'){
                makeOption1(null,messageid,id,replied);
            }
            if(value=='2'){
        ...
        function makeOption50(json){
            var messageType=$("#selectMessage2").val();
            var url=path+"/message/viewMessageAddmymessage.do";
            OrderGetOrders=openMyDialog({title: '查看消息',
                content: 'url:'+url+'?transactionid='+json.transactionid+'&itemid='+json.itemid+'&recipientid='+json.recipientid+'&sender='+json.sender+'&messageid='+json.messageid+'&ebayId='+json.ebayId+"&messageType="+json.messagetype,
                icon: 'succeed',
                width:1200,
                height:850,
                lock:true
        ...
        function selectMessage3(count,name){
            var days=$("span[scop=message3]");
            for(var i=0;i<days.length;i++){
                if(i==(count)){
                    $(days[i]).attr("class","newusa_ici");
                    $("#selectMessage2").val(name);
                }else{
                    $(days[i]).attr("class","newusa_ici_1");
                }
            }
        ...
        function initSearchInput(bs){
            ...
            mySelect2I([{url:_url,doitAfterSelect:queryMessage,
        ...
        function initSearchInput1(bs){
            ...
            mySelect2I([{url:_url,doitAfterSelect:queryMessage1,
        ...
        function initSearchInput2(bs){
            ...
            mySelect2I([{url:_url,doitAfterSelect:queryMessage2,
       ...
       /* <input type="hidden" id="selectAmount"/>
        ...
        function queryMessage2(){
            ...
            var url1="/message/ajax/loadMessageAddmymessageList.do?replied="+replied;
            if(messageType!='4'){
                url1="/message/ajax/loadMessageAddmymessageList.do?";
            }
            $("#MessageGetmymessageListTable2").initTable({
                url:path + url1,
                columnData:[
                    /*{title:"",name:"pictureUrl",width:"2%",align:"left",format:makeOption4},*/
                    {title:"主题",name:"read",width:"8%",align:"center",format:makeOption2,click:makeOption50},
                    {title:"From > to",name:"sender",width:"8%",align:"center",format:makeOption3,click:makeOption50},
                    {title:"SKU",name:"sku",width:"8%",align:"center",click:makeOption50},
                    {title:"修改时间",name:"createTime",width:"8%",align:"center",click:makeOption50}/*,
            ...
            refreshTable3(amount,status,day,type,content,starttime,endtime,messageType);
        }
        ...
        <div class="new">
            ...
            <div class="Contentbox">
                ...
                <form id="selectForm2">
                    ...
                    <input type="hidden" id="selectMessage2">
                </form>
...
<select id="typeQuery"  name="type" onchange="chageOldDom(this)" style="color: #737FA7;width: 80px;float: left">
    ...
    <option value="3">SKU</option>
                            ...
                            <div class="vsearch">
                                <input id="content" name="" type="text"  style="width:200px;float: left"  multiple class="multiSelect"><input onclick="queryMessage();" style="display: none;" name="newbut" type="button" class="key_2"></div>
                        </div>
                        <div class="newds">
...
<select id="typeQuery1" name="type" onchange="chageOldDom1(this)" style="color: #737FA7;width: 80px;float: left">
    <option value=""  selected="selected">选择类型</option>
    ...
    <option value="3">SKU</option>
                                ...
                                <div class="vsearch">
                                    <input id="content1" name="" type="text" style="width:200px;float: left"  multiple class="multiSelect"><input onclick="queryMessage1();" style="display: none;" name="newbut" type="button" class="key_2"></div>
                            </div>
                            <div class="newds">
                ...
                <div style="display: none;" id="con_menu_3">
                        <!--综合开始 -->
                        <div class="new_usa" style="margin-top:20px;">
                            <li class="new_usa_list"><span class="newusa_i" style="width: 75px;">销售状态：</span><a href="javascript:void(0)"><span class="newusa_ici" scop="message3" onclick="selectMessage3(0,null);">全部&nbsp;</span></a><a href="javascript:void(0)"><span class="newusa_ici_1" scop="message3" onclick="selectMessage3(1,'4')">发送失败&nbsp;</span></a><a href="javascript:void(0)"><span class="newusa_ici_1" scop="message3" onclick="selectMessage3(2,'2')">自动消息&nbsp;</span></a></li>
                            ...
                            <span class="newusa_i" style="width: 75px;">搜索内容：</span>
                                <span id="sleBG" style="width:82px;background-position: 67px 10px;">
                    <span id="sleHid" style="width: 80px;">
<select id="typeQuery2" name="type"  onchange="chageOldDom2(this)" style="color: #737FA7;width: 80px;float: left">
    <option value=""  selected="selected">选择类型</option>
    <option value="1">发件人</option>
    <option value="2">主题</option>
    <option value="3">SKU</option>
                                ...
                                <div class="vsearch">
                                    <input id="content2" name="" type="text"  style="width:200px;float: left"  multiple class="multiSelect"><input onclick="queryMessage2();" style="display: none;" name="newbut" type="button" class="key_2"></div>
                            </div>
                            <div class="newds">
                                <div class="newsj_left" style="margin-right:15px;">
                                 <%--   <span class="newusa_ici_del_in"><input name="checkbox" id="checkbox" type="checkbox" onclick="Allchecked(this);"></span>
