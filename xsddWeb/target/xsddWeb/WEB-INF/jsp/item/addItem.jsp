...
<script>
    ...
    var title = '${itemTitle}';
    var itemPrice = '${item.startprice==null?"0":item.startprice}';
    var itemCount ='${item.quantity}' ;
    var subtitle = "${item.subtitle}";
    var titleLength = '${titleLength}';
    ...
    $(document).ready(function () {
        ...
        var secondaryCategoryid='${item.secondaryCategoryid}';
        $("#oneAttr").show();
        $("#twoAttr").hide();
        $("#Auction").hide();
        ...
        $("#priceMessage").show()
        if(secondaryCategoryid!=null&&secondaryCategoryid!=""){
            $("#secodaryCate").show();
            getSecondaryCategoryName(secondaryCategoryid,site);
        }else{
            $("#secodaryCate").hide();
        }
        ...
        if(ebayAccount==null||ebayAccount==""){
            ...
            ebayAccount = $("input[name='ebayAccounts']:checked").val();
        }
        ...
        var str = "";
        str += "<tr title='鼠标移至本行前方可拖拽排序' style='height: 32px;'><td class='dragHandle' width='15px;'></td>";
...
*/
            if(i==attrValue.keys.length-1){
                morePicid+='{"a":"'+$("#moreAttrs tr:eq(0) th:eq(4)").find("input").val() + 'this' + replaceTSFH(attrValue.get(attrValue.keys[i]))+'","b":"'+replaceTSFH(attrValue.get(attrValue.keys[i]))+'"}';
            }else{
                morePicid+='{"a":"'+$("#moreAttrs tr:eq(0) th:eq(4)").find("input").val() + 'this' + replaceTSFH(attrValue.get(attrValue.keys[i]))+'","b":"'+replaceTSFH(attrValue.get(attrValue.keys[i]))+'"},';
            }
        }
        ...
        picstr += '<li title="可拖拽排序"><div style="position:relative"><input type="hidden" name="pic_mackid" value="${lipic.attr1}"/><input type="hidden" name="PictureDetails_' + ebayAccount + '.PictureURL" value="${lipic.value}">' +
        ...
        $("#hiddenText").html("${pics.tamname}");
        /*if($('#picturemore_' + replaceTSFH($("#hiddenText").html())).find("[name*='VariationSpecificValue_']").length==0) {
            console.log("${pics.tamname}");
            $("#picturemore_" + replaceTSFH($("#hiddenText").html())).append("<input type='hidden' name='VariationSpecificValue_" + replaceTSFH($("#hiddenText").html()) + "' value="+$("#hiddenText").html()+">");
            //$("#picturemore_" + replaceTSFH($("#hiddenText").html())).append("<input type='hidden' name='VariationSpecificValue_" + replaceTSFH($("#hiddenText").html()) + "' value="+$("#hiddenText").html()+">");
        }*/
        <c:forEach items="${pics.litam}" var="pi">
        $("#picturemore_"+replaceTSFH($("#hiddenText").html())).append("<li><div style='position:relative'><input type='hidden' name='pic_mackid_more' value='${pi.attr1}'/><input type='hidden' name='"+replaceTSFH($("#hiddenText").html())+"' value='${pi.value}'><img src="+chuLiPotoUrl('${pi.value}')+" height='80' width='78' /> <div style='text-align: right;background-color: dimgrey;'><img src='"+path+"/img/newpic_ico.png' onclick='removeThis(this)'></div></div></li>");
        </c:forEach>
        </c:forEach>
       ...
       changeBackcolour();
        ...
        if(_sku==null||_sku=="") {
            if($("ul[id^='picture_']")!=null&&$("ul[id^='picture_']").html()!=undefined){
            }else {
                setTimeout(function () {
                    isShowPicLink();
                }, 1000);
        ...
        if(ebayaccount!=''){
            ...
            if($("input[type='checkbox'][name='ebayAccounts']:checked").val()!=undefined){
                var ebayAcc="&ebayId="+$("input[type='checkbox'][name='ebayAccounts']:checked").val();
                loadShippingDeails(ebayAcc);
                getPayIdStr(ebayAcc);
            }else{
                alert("请先绑定ebay账号及paypal账号，如果未绑定，将无法以刊登商品！");
            }
        }

        setTimeout(function(){
            for(var i=0;i<attrValue.keys.length;i++){
                if($('#picturemore_' + replaceTSFH(attrValue.keys[i])).find("[name*='VariationSpecificValue_']").length==0){
                    $('#picturemore_' + replaceTSFH(attrValue.keys[i])).append("<input type='hidden' name='VariationSpecificValue_" + replaceTSFH(attrValue.keys[i]) + "' value=''>");
                    $("input[name='VariationSpecificValue_"+replaceTSFH(attrValue.keys[i])+"']").val(attrValue.keys[i]);
                }
            }
        },200);
        compar2SepcAttr();
...
<div class="new">
    ...
    <li>
        ...
        <c:forEach items="${ebayList}" var="ebay">
            <em style="color:#48a5f3"><input type="checkbox" name="ebayAccounts" value="${ebay.id}"
                                             ...
                                             onchange="selectAccount(this)">${fn:toUpperCase(ebay.ebayNameCode)}</em>
        </c:forEach>
    ...
    <div id="titleDiv">
        ...
        <li style="display:none;height: 35px;">
            <dt>子标题</dt>
            <div class="new_left">
                <input type="text" name="SubTitle_${item.ebayAccount}" style="width:400px;" class="validate[maxSize[55]] form-control"
                       id="SubTitle"
    ...
    <li style="height: 58px;">
        <dt>第一分类</dt>
        <div class="new_left">
            <div id="showErrorCate" style="color: red;"></div>
            ...
            <b class="new_button"><a data-toggle="modal" href="javascript:void(0)" onclick="selectType(1)">选择分类</a></b>

            <b class="new_button"><a data-toggle="modal" href="javascript:void(0)" onclick="queryType(1)">搜索分类</a></b>
        </div>
    </li>
    <li id="PrimaryCategoryshow" style="padding-left: 100px;height: 20px;"></li>
    <div id="secodaryCate" class="panel" style="display: block;">
        <li style="padding-top:9px;">
            <dt>第二分类</dt>
            <div class="new_left">
                <input type="text" onblur="addSecondaryCatename(this,'${item.site}')" style="width:300px;" id="SecondaryCategory" class="form-control" name="SecondaryCategory.CategoryID"
                       title="SecondaryCategory.CategoryID" value="${item.secondaryCategoryid}">
                <b class="new_button"><a data-toggle="modal" href="javascript:void(0)" onclick="selectType(2)">选择分类</a></b>

                <b class="new_button"><a data-toggle="modal" href="javascript:void(0)" onclick="queryType(2)">搜索分类</a></b>
                选取第二分类，对应的刊登费用会有所增加
            </div>
        </li>
        <li id="SecondaryCategoryshow" style="padding-left: 100px;height: 20px;"></li>
    </div>
    <li class="flip" style=" padding-left:260px;padding-bottom: 20px;height: 5px;" onclick="showSubCategory(this)"><img src="/xsddWeb/img/new_list_ico.png" class="abc"></li>
    <div class="panel"  id="StoreCategoryshow">
        <li style="background:#ffffff; padding-top:9px;">
            <dt>店铺分类</dt>
            <div class="new_left">
                <input type="hidden" name="storeCategoryId" id="storeCategoryId" value="${tsc.id}"/>
                <input type="hidden" style="width:300px;" class="form-control" id="Storefront.storeCategoryID" name="Storefront.storeCategoryID"
                       title="Storefront.storeCategoryID" value="${tsc.storeCategoryId}">
                <input type="text" style="width:300px;" class="form-control" id="Storefront.storeCategoryName" name="Storefront.storeCategoryName"
                       title="Storefront.storeCategoryName" value="${tsc.storeCategoryName}" readonly>
                <b class="new_button"><a data-toggle="modal" href="javascript:void(0)" onclick="selectStoreType()">选择分类</a></b>
            </div>
        </li>
    </div>
    <h1>
        ...
        <span style="font-weight: 100;padding-left: 30px;">
            已选择图片：<span id="picNumber">0</span>张｜最多<span id="totalPic">12</span>张图片&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
...
<span id="showPics">
    ...
    <li style="height: auto;padding-top: 10px;">
...
<%--        <h1 style="padding-left: 50px;">拍买</h1>--%>
        ...
        <li>
            <dt>保留价</dt>
            <div class="new_left">
                <input type="text" name="ReservePrice" style="width:300px;" id="ReservePrice" class="validate[required] form-control"  onkeypress='return inputNUMAndPoint(event,this,2)' value="0.00">
        ...
        <li>
            <dt>一口价</dt>
            <div class="new_left">
                <input type="text" name="BuyItNowPrice" style="width:300px;" id="BuyItNowPrice" class="validate[required] form-control"  onkeypress='return inputNUMAndPoint(event,this,2)' value="0.00">
                <label name="curName" style="border: 1px solid #cccccc;background-color: #eee;line-height: 26px;height: 26px;
                position: absolute;text-align: center;width: 110px;left: 340px;-webkit-border-top-right-radius: 5px;-webkit-border-bottom-right-radius: 5px;"></label>
            </div>
        </li>
        <li>
            <dt>销售比基数</dt>
            <div class="new_left">
                <input type="text" name="ListingScale" id="ListingScale" class="validate[required] form-control" style="width:100px;" value="0">
            </div>
        ...
        <li style="display: none;">
            <dt>是否单物品</dt>
                <select name="ListingFlag">
                    <option value="0" selected="true">单独物品</option>
    ...
    <div id="oneAttr" style="display: none;">
        <li>
            <dt>商品价格</dt>
            <div class="new_left">
                <input type="text" name="StartPrice.value_${item.ebayAccount}" id="productPrice" style="width:300px;" onkeypress='return inputNUMAndPoint(event,this,2)'
        ...
        <li id="abc_pai">
            <dt>商品数量</dt>
            <div class="new_left">
                <input type="text" style="width:300px;" id="productQuantity" onkeypress='return inputOnlyNUM(event,this)' class="validate[required] form-control"
...
<input type="hidden" name="applicationdata" id="applicationdata" value="${item.applicationdata}"/>
...
<div id="new_view" class="new_view" style="position: fixed;bottom: 0px;overflow: visible" >
    ...
    <c:if test="${item.isFlag==null||item.isFlag==''}">
        <li ><a href="javascript:void(0)" onclick="checkPic(this,'all')">立即刊登</a></li>
        <li id="houtaikd"><a  href="javascript:void(0)" onclick="checkPic(this,'Backgrounder')">后台刊登</a></li>
    </c:if>
    <li><a href="javascript:void(0)" onclick="checkPic(this,'save')">保存范本</a></li>
    <c:if test="${item.isFlag==null||item.isFlag==''}">
        <li><a href="javascript:void(0)" name="timeSave" onclick="selectTimer(this)">定时</a></li>
    </c:if>
    <li><a href="javascript:void(0)" onclick="checkPic(this,'othersave')">另存为新范本</a></li>
    <c:if test="${item.itemId!=null||item.isFlag==''}">
        <li><a href="javascript:void(0)" onclick="checkPic(this,'updateListing')">更新在线刊登</a></li>
    </c:if>
    <li><a onclick="checkPic(this,'previewItem')" href="javascript:void(0)" target="_blank">预览</a></li>

    <li><a onclick="checkPic(this,'checkEbayFee')" href="javascript:void(0)">检查ebay费</a></li>
