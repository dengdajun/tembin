...
<head>
    ...
    <script>
        ...
        $(document).ready(function() {
            var dataType = '${inventory.dataType}';
            $("select[name='dataType']").find("option[value='" + dataType + "']").attr("selected", true);
            $("div[name='remark']").hide();
            if($("select[name='dataType']").find("option:selected").text()=="出口易"){
                $("#cky").show();
            }else if($("select[name='dataType']").find("option:selected").text()=="第四方"){
                $("#dsf").show();
            }else if($("select[name='dataType']").find("option:selected").text()=="四海邮"){
                $("#shy").show();
            }
...
<div class="modal-body">
    <form class="form-horizontal" role="form" id="payPalForm">
        <table width="90%" border="0" style="margin-left:40px;">
            <tr>
                <td height="28" align="right">库存类型：</td>
                <td height="28">
