...
function onloadTable(urls){
    ...
    if(urls.indexOf("flag=3")!=-1){
        clod = [
            ...
            {title:"<span onclick='orderList(this)' style='cursor: pointer;' colu='ebay_name' val='0'>eBay账户</span>",name:"ebayaccountname",width:"12%",align:"left"},
...
function itemstatus(json){
    var htm = "";
    if(json.listingWay=="1"&&(json.itemId==null||json.itemId=="")&&json.timerFlag=="0"&&json.timerCheckFlag=="0"){
        htm="<img title='定时时间："+json.defaultTimer+"' name='statusimg' src='"+path+"/img/timer.png'>";
    }else{
...
function toFolder(idStr){
    var mytoFolder = openMyDialog({title: '选择移动到的文件夹',
        content: 'url:'+path+'/selecttofolder.jsp?foldertype=modelFolder',
        icon: 'tips.gif',
        width: 400,
        button: [
            {
                ...
                button: [
                    {
                        ...
                        callback: function (iwins, enter) {
                            ...
                            if(cc==1){
                                for(var i=0;i<iwins.document.getElementsByName("ebayAccount").length;i++){
                                    if($(iwins.document.getElementsByName("ebayAccount")[i]).prop("checked")){
                                        ebayaccount+=$(iwins.document.getElementsByName("ebayAccount")[i]).val()+",";
                                    }
                                }
                            }else{
                                for(var i=0;i<iwins.parent.document.getElementsByName("ebayAccount").length;i++){
                                    if($(iwins.parent.document.getElementsByName("ebayAccount")[i]).prop("checked")){
                                        ebayaccount+=$(iwins.parent.document.getElementsByName("ebayAccount")[i]).val()+",";
                                    }
                                }
                            }
                            if(ebayaccount==""){
                            ...
                            var url = path + "/ajax/copyItem.do?ids=" + idStr+"&ebayaccount="+ebayaccount.substr(0,ebayaccount.length-1);
                            $().invoke(url, {},
                                [function (m, r) {
                                    alert(r);
                                    Base.token();
...
function renameItem(idStr){
    ...
    itemNamePage = openMyDialog({title: '请输入名称',
        content: 'url:'+path+'/renameitem.jsp?itemName='+name,
        icon: 'tips.gif',
        width: 400,
        button: [
            {
                name: '确定',
                callback: function (iwins, enter) {
                    var fileName = itemNamePage.content.document.getElementsByName("fileName")[0].value;
                    if(fileName==""){
                        alert("请输入名称！");
                        return false;
                    }
                ...
                for(var i=0;i<id.length;i++){
                    for(var j=0;j< r.length;j++){
                        if(id[i]==r[j].id){
                            ...
                            $("#"+id[i]).find("img[name='statusimg']").attr("title","定时时间："+$("#timerListing").val());
                            $("#itemTable").refresh();
