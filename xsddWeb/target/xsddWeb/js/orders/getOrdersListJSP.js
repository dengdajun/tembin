...
function getOrderList1(){
    $("#OrderGetOrdersListTable1").initTable({
        ...
        columnData:[
            ...
            {title:"图片/交易号",name:"orderid",width:"10%",align:"center",format:makeOption2},
            {title:"<div style='width: 100%' align='center'>买家/SKU/承运商/追踪号</div>",name:"orderid",width:"16%",align:"left",format:makeOption3},
            {title:"<a href=\"javascript:void(0)\" onclick=\"orderByTransactionPrice(this,'asc3');\"><font style=''>站点</font></a>",name:"itemSite",width:"5%",align:"center",format:makeOption20},
            {title:"<a href=\"javascript:void(0)\" onclick=\"orderByTransactionPrice(this,'asc');\"><font style=''>售价</font></a>",name:"itemUrl",width:"8%",align:"center",format:makeOption6},
            {title:"<a href=\"javascript:void(0)\" onclick=\"orderByTransactionPrice(this,'desc1');\"><font style=''>售出日期</font></a>",name:"createdtime",width:"5%",align:"center"},
            {title:"更新时间",name:"lastmodifiedtime",width:"5%",align:"left"},
            {title:"<a href=\"javascript:void(0)\" onclick=\"orderByTransactionPrice(this,'asc2');\"><font style=''>数量</font></a>",name:"quantitypurchased",width:"3%",align:"center"},
            {title:"<a href=\"javascript:void(0)\" onclick=\"orderByTransactionPrice(this,'asc4');\"><font style=''>EBAY账号</font></a>",name:"selleruserid",width:"8%",align:"center"},
            {title:"状态",name:"shipped",width:"3%",align:"left",format:makeOption4},
            {title:"<div style='width: 100%' align='center'>操作</div>",name:"option1",width:"8%",align:"center",format:makeOption1}
        ],
        selectDataNow:false,
        isrowClick:false,
        showIndex:false
...
function orderByTransactionPrice(obj,orderby){
    ...
    $("#"+tableid).initTable({
        ...
        columnData:[
            ...
            {title:"<div style='width: 100%' align='center'>买家/SKU/承运商/追踪号</div>",name:"orderid",width:"13%",align:"left",format:makeOption3},
...
function refreshTable22(){
    $("#"+orderListTable).refresh({"bedDetailVO.deptId":"", "isTrue":0});
}
...
var hs="";
hs="<li style=\"height:25px;\"  onclick=selectOperation('"+json.orderid+"','"+json.transactionid+"','"+json.ebayId+"',this,'"+json.id+"'); value='1' doaction=\"readed\" >查看详情</li>";
hs+="<li style=\"height:25px;\"  onclick=selectOperation('"+json.orderid+"','"+json.transactionid+"','"+json.ebayId+"',this,'"+json.id+"'); value='2' doaction=\"look\" >上传跟踪</li>";
hs+="<li style=\"height:25px;\"  onclick=selectOperation('"+json.orderid+"','"+json.transactionid+"','"+json.ebayId+"',this,'"+json.id+"'); value='3' doaction=\"look\" >发送消息</li>";
    var today=new Date();
    var paidtime=json.paidtime;
    if(paidtime!=null&&paidtime!=''){
        paidtime = paidtime.replace(/-/g,"/");
        var paidtime1=new Date(paidtime);
        paidtime1.setDate(paidtime1.getDate()+65);
        if(paidtime1>=today){
            hs+="<li style=\"height:25px;\"  onclick=selectOperation('"+json.orderid+"','"+json.transactionid+"','"+json.ebayId+"',this,'"+json.id+"'); value='4' doaction=\"look\" >退款功能</li>";
        }
    }
hs+="<li style=\"height:25px;\"  onclick=selectOperation('"+json.orderid+"','"+json.transactionid+"','"+json.ebayId+"',this,'"+json.id+"'); value='5' doaction=\"look\" >添加备注</li>";
hs+="<li style=\"height:25px;\"  onclick=selectOperation('"+json.orderid+"','"+json.transactionid+"','"+json.ebayId+"',this,'"+json.id+"'); value='6' doaction=\"look\" >删除</li>";
hs+="<li style=\"height:25px;\"  onclick=selectOperation('"+json.orderid+"','"+json.transactionid+"','"+json.ebayId+"',this,'"+json.id+"'); value='7' doaction=\"look\" >查看日志</li>";
var pp={"liString":hs};
...
function selectOperation(orderid,transactionid,selleruserid,obj,id){
    ...
    if(value=="2"){
        modifyOrderStatus(transactionid,selleruserid,orderid,id);
    }
    if(value=="3"){
        sendMessage(transactionid,selleruserid,id,orderid);
    }
    if(value=="4"){
        issueRefund(transactionid,selleruserid,orderid);
    }
    if(value=="5"){
        addComment1(id);
    }
...
function viewSystemlog(id){
    var url=path+'/order/viewSystemlog.do?id='+id;
...
function sendMessage(transactionid,ebayId,id,orderId){
    var url=path+'/order/initOrdersSendMessage.do?transactionid='+transactionid+'&ebayId='+ebayId+'&id='+id+'&orderId='+orderId;
...
function issueRefund(transactionid,ebayId,orderId){
    var url=path+'/order/initIssueRefund.do?transactionid='+transactionid+'&ebayId='+ebayId+'&orderId='+orderId;
...
/*    if(json.message==null||json.message==""){
    ...
    htm+="</span><span style=\"width:100%; float:left; color:#8BB51B\">"+skuShow+"</span>" +
...
function makeOption4(json){
    ...
    if(json.orderstatus=='Active'&&json.status=='Incomplete'){
        ...
        if(json.refundstatus=='Succeeded'){
            htm="<img title=\"已退款\" src=\""+imgurl1+"refund.png\" >";
        }else if(((json.shipmenttrackingnumber!=""&&json.shipmenttrackingnumber!=null)||(json.shippedtime!=""&&json.shippedtime!=null))&&json.paidtime!=""&&json.paidtime!=null){
            htm="<img title=\"发货\" src=\""+imgurl1+"a2.png\" >";
        }else if(json.paymentstatus=='Succeeded'&&json.paidtime!=null&&json.paidtime!=""){
    ...
    if(json.status=='Complete'&&json.orderstatus=='Completed'){
        /*if(json.feedbackMessage!=""){
            htm="<img title=\"签收\" src=\""+imgurl1+"a4.png\" >";
        }else*/
        if(json.refundstatus=='Succeeded'){
...
function submitCommit(){
    $("#OrderGetOrdersListTable").initTable({
        ...
        columnData:[
            {title:"",name:"ch",width:"1%",align:"top",format:makeOption5},
            {title:"图片/交易号",name:"orderid",width:"10%",align:"left",format:makeOption2},
            {title:"<div style='width: 100%' align='center'>买家/SKU/承运商/追踪号</div>",name:"orderid",width:"20%",align:"left",format:makeOption3},
...
function viewOrder(id,transactionid,ebayId){
    var url=path+'/order/viewOrderGetOrders.do?orderid='+id+'&transactionid='+transactionid+'&ebayId='+ebayId;
...
function modifyOrderStatus(id,ebayId,orderId,primId){
    var url=path+'/order/modifyOrderStatus.do?primId='+primId+'&transactionid='+id+'&ebayId='+ebayId+'&orderId='+orderId;
...
function query1(){
    ...
    var content=$("#content1").select2("data")!=null?$("#content1").select2("data").text:"";
...
function query2(){
    ...
    var content=$("#content2").select2("data")!=null?$("#content2").select2("data").text:"";
...
function query3(){
    ...
    var content=$("#content3").select2("data")!=null?$("#content3").select2("data").text:"";
...
function query4(){
    ...
    var content=$("#content4").select2("data")!=null?$("#content4").select2("data").text:"";
...
function query5(){
    ...
    var content=$("#content5").select2("data")!=null?$("#content5").select2("data").text:"";
...
function query6(){
    if(searchi>=5){
        ...
        var typeQ=$("#typeQ"+searchi).val();
        var itemType=$("#itemType"+(searchi+1)).val();
        var content=$("#content"+(searchi+1)).select2("data")!=null?$("#content"+(searchi+1)).select2("data").text:"";
        var folderId=$("#menu"+(searchi+1)).attr("name2");
        var starttime1=$("#starttime"+searchi).val();
        var endtime1=$("#endtime"+searchi).val();
        var statusQ=$("#statusQ"+searchi).val();
        var accountQ=$("#accountQ"+searchi).val();
        var table1=$("#OrderGetOrdersListTable"+(searchi+1));
        refreshTable6(table1,countryQ,typeQ,daysQ,itemType,content,folderId,starttime1,endtime1,statusQ,accountQ);
    }
...
function downloadOrders(n){
    ...
    //-------------------------------------
    var daysQ=$("#daysQ"+orderPageNum).val();
    var countryQ=$("#countryQ"+orderPageNum).val();
    var typeQ=$("#typeQ"+orderPageNum).val();
    var itemType=$("#itemType"+orderPageNum).val();
    var content=$("#content"+orderPageNum).select2("data")!=null?$("#content"+orderPageNum).select2("data").text:"";
    var starttime=$("#starttime"+orderPageNum).val();
    var endtime=$("#endtime"+orderPageNum).val();
    var statusQ=$("#statusQ"+orderPageNum).val();
    var accountQ=$("#accountQ"+orderPageNum).val();
    if(content!=""&&itemType!="title"){
        content = content.trim();
    }
    var folderId="";
    if(orderPageNum>5){
        folderId=$("#menu"+n).attr("name2");
    }
    //-----------------------------------
    var url=path+"/order/downloadOrders.do?status="+status+"&daysQ="+daysQ+"&countryQ="+countryQ+"&typeQ="+typeQ+"&itemType="+itemType+"&content="+content+"&starttime="+starttime+"&endtime="+endtime+"&statusQ="+statusQ+"&accountQ="+accountQ+"&folderId="+folderId;
    window.open(url);
}
    ...
    $().invoke(url,null,
        [function(m,r){
            ...
            for(var i=0;i<6;i++){
                ...
                $(dt).attr("onclick","setTab('menu',1,6)");
            }
            var htm="";
    ...
    for(var i=1;i<=n;i++){
        ...
        if(i>5&&i==cursel){
            ...
            for(var j=1;j<=accounts.length;j++){
                ...
                if(j==1){
                    $(accounts[j-1]).attr("onclick","selectAccount1("+i+","+j+","+null+");");
                }else{
            ...
            $(itemType1).attr("id","itemType"+i);
            $(itemType1).removeAttr("onchange");
            $(lastdiv).find("div[id=s2id_content4]").remove();
            var bs1=$(itemType1).val();
            $(itemType1).change(function(){
                var bs=$(itemType1).val();
                initSearchInput5(bs,(i-2),"menu"+(i-1),"OrderGetOrdersListTable"+(i-1));
            });
...
var orderListTable="OrderGetOrdersListTable1";
var orderPageNum=1;
//--------------------------------------------------------------------------
function setTab(name,cursel,n){
    orderListTable="OrderGetOrdersListTable"+cursel;
    orderPageNum=cursel;
...
function initSearchInput(bs){
    ...
    mySelect2I([{url:_url,doitAfterSelect:query1,
        data:{currInputName:"content"},bs:"#content1",multiple:false,maping:_map}]);
}

function chageOldDom1(obj){
    cleanInput();
    initSearchInput1($(obj).val());
    $("input[id='content2']").val('');
}
/**初始化select2搜索框*/
function initSearchInput1(bs){
    ...
    mySelect2I([{url:_url,doitAfterSelect:query2,
        data:{currInputName:"content"},bs:"#content2",multiple:false,maping:_map}]);
}

function chageOldDom2(obj){
    cleanInput();
    initSearchInput2($(obj).val());
    $("input[id='content3']").val('');
}
/**初始化select2搜索框*/
function initSearchInput2(bs){
    var _url="";
    var _map={};
    if(bs=='sku'){
        _url=path+"/informationType/ajax/loadOrgIdItemInformationList.do";
    ...
    mySelect2I([{url:_url,doitAfterSelect:query3,
        data:{currInputName:"content"},bs:"#content3",multiple:false,maping:_map}]);
}

function chageOldDom3(obj){
    cleanInput();
    initSearchInput3($(obj).val());
    $("input[id='content4']").val('');
}
/**初始化select2搜索框*/
function initSearchInput3(bs){
    var _url="";
    var _map={};
    if(bs=='sku'){
        _url=path+"/informationType/ajax/loadOrgIdItemInformationList.do";
    ...
    mySelect2I([{url:_url,doitAfterSelect:query4,
        data:{currInputName:"content"},bs:"#content4",multiple:false,maping:_map}]);
}

function chageOldDom4(obj){
    cleanInput();
    initSearchInput4($(obj).val());
    $("input[id='content5']").val('');
}
/**初始化select2搜索框*/
function initSearchInput4(bs){
    var _url="";
    var _map={};
    if(bs=='sku'){
        _url=path+"/informationType/ajax/loadOrgIdItemInformationList.do";
    ...
    mySelect2I([{url:_url,doitAfterSelect:query5,
        data:{currInputName:"content"},bs:"#content5",multiple:false,maping:_map}]);
}
var searchi=0;
function initSearchInput5(bs,n,id1,table) {
    searchi=n;
    var _url = "";
    var _map = {};
    if (bs == 'sku') {
    ...
    mySelect2I([
        {url: _url, doitAfterSelect: query6,
            data: {currInputName: "content"}, bs: "#content"+(n+1), multiple: false, maping: _map}]);
}
function menuFix() {
