...
$(document).ready(function(){
   ...
   // doContainer();
    ...
    if(ebaySize==null||ebaySize==0||ebaySize=='0'){
        alert("您好像还没有绑定ebay和paypal，请到'系统设置-账号绑定'进行设置，如有疑问，请加群244373066");
    }
...
function queryIndexEbayList(){
    var msg="";
    if(isMainAccount=='0'){
        msg="您还没有绑定ebay账号！<button onclick='dirPage()'>立即绑定</button>";
    }else{
        msg="请使用主账号分配ebay帐号！";
    }
    $("#ebay_indexdiv").initTable({
        url:"/xsddWeb/user/queryEbaysForCurrUser.do?hsource=indexPage",
        columnData:[
            {title:"代码",name:"ebayNameCode",width:"8%",align:"left"},
            {title:"ebay账户",name:"ebayName",width:"8%",align:"left"},
            {title:"信用评价",name:"",width:"8%",align:"left",format:function(json){
                return json.feedBackNum+" * "+json.perFeedBack
            }},
            {title:"密钥有效期",name:"op",width:"8%",align:"left",format:mCanUseDate},
            {title:"状态",name:"op",width:"8%",align:"left",format:makeStatus}
        ],
        selectDataNow:false,
        rowClickMethod:function(j,o){
            window.open("http://www.ebay.com/usr/"+ j.ebayName)
        },
        isrowClick:true,
        showIndex:false,
        sysParm: {"jsonBean.pageNum": 1, "jsonBean.pageCount": 1000},
        showDataNullMsgContext:msg,
        onlyFirstPage:true
    });
...
function makeStatus(json){
    ...
    }else if(json.ebayStatus==0 || json.ebayStatus=='0'||json.ebayStatus==2||json.ebayStatus=='2'){
        imgurlpr+="new_no.png";
    }else{
        imgurlpr+="";
    }
...
function queryPaypalList(){
    ...
    $("#indexPayPal").initTable({
        url:"/xsddWeb/paypal/queryPaypalListReport.do",
        columnData:[
...
/**转向帐号绑定*/
function dirPage(){
    location.href=path+"/user/ebayAccountManager.do"
}
