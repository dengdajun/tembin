...
public class PaypalController extends BaseAction {
    ...
    public void getPaypalYanZheng(CommonParmVO commonParmVO) throws Exception {
        ...
        PaypalVO acc = payPalService.getPaypalBalance(commonParmVO.getId());
        String r="验证失败,请到paypal后台给予相应的权限";
    ...
    @ResponseBody
    public void queryPaypalList(CommonParmVO2 commonParmVO){
        Map map =new HashMap();
    ...
    @RequestMapping("queryPaypalListReport.do")
    @ResponseBody
    /**查询paypalList 分页*/
    public void queryPaypalListReport(CommonParmVO2 commonParmVO){
        Map map =new HashMap();
        PageJsonBean jsonBean = commonParmVO.getJsonBean();
        Page page = jsonBean.toPage();
        //List<UsercontrollerPaypalAccount> paypalAccounts = payPalService.queryPayPalList(map, page);
    ...
    public void addPayPalAccount(UsercontrollerPaypalAccount paypalAccount){
        ...
        Asserts.assertTrue(StringUtils.isNotBlank(paypalAccount.getPaypalAccount()),"帐号不能为空！");
