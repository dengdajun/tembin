...
public class PriceTrackingController extends BaseAction{
    ...
    //查看竞争对手
    @RequestMapping("/viewPricingTracking.do")
    public ModelAndView viewPricingTracking(HttpServletRequest request,HttpServletResponse response,ModelMap modelMap){
        String sku=request.getParameter("sku");
        String ebayAcount=request.getParameter("ebayAcount");
        modelMap.put("sku",sku);
        modelMap.put("ebayAcount",ebayAcount);
        return forword("/priceTracking/viewPricingTracking",modelMap);
    }
    //添加竞争对手
    ...
    @RequestMapping("/ajax/viewPricingTracking1.do")
    @ResponseBody
    public void viewPricingTrackingList1(CommonParmVO commonParmVO,HttpServletRequest request) throws Exception {
        String sku=request.getParameter("sku");
        String ebayAcount=request.getParameter("ebayAcount");
        Page page=new Page();
        ...
        page.setPageSize(10);
        Map map=new HashMap();
        List<PriceTrackingQuery> priceTrackings=new ArrayList<>();
        if(StringUtils.isNotBlank(sku)){
            ...
            if(flag){
                ...
                List<UsercontrollerUserExtend> orgUsers1=new ArrayList<>();
                UsercontrollerUserExtend userExtend=new UsercontrollerUserExtend();
                userExtend.setUserId((int) sessionVO.getId());
                orgUsers1.add(userExtend);
                map.put("orgUsers",orgUsers1);
            }
            map.put("sku",sku);
            map.put("ebayAcount", HtmlUtils.htmlEscape(ebayAcount));
            priceTrackings=iTradingPriceTracking.selectPriceTrackingByView(map,page);
            if(priceTrackings!=null&&priceTrackings.size()>0){
                AjaxSupport.sendSuccessText("","true");
            }else{
                AjaxSupport.sendSuccessText("","false");
            }
        }else{
            AjaxSupport.sendSuccessText("","false");
        }
    ...
    @RequestMapping("/ajax/viewPricingTracking.do")
    @ResponseBody
    public void viewPricingTrackingList(CommonParmVO commonParmVO,HttpServletRequest request) throws Exception {
        String sku=request.getParameter("sku");
        String ebayAcount=request.getParameter("ebayAcount");
        PageJsonBean jsonBean=commonParmVO.getJsonBean();
        ...
        if(StringUtils.isNotBlank(sku)){
            ...
            map.put("sku",sku);
            map.put("ebayAcount",ebayAcount);
            priceTrackings=iTradingPriceTracking.selectPriceTrackingByView(map,page);
            TradingListingData listingData=iTradingListingData.selectBySkuAndEbayAcount(sku,ebayAcount);
            if(listingData!=null){
                for(PriceTrackingQuery priceTracking:priceTrackings){
                    priceTracking.setItemid1(listingData.getItemId());
                    priceTracking.setItemPrice1(String.valueOf(listingData.getPrice()));
                    priceTracking.setSku(listingData.getSku());
                }
            }
        }
        page.setTotalCount(priceTrackings.size());
        jsonBean.setList(priceTrackings);
    ...
    public void saveAutoPriceListingDate(CommonParmVO commonParmVO,HttpServletRequest request) throws Exception {
        ...
        if(StringUtils.isNotBlank(id)){
            ...
            for(String itemid:itemids1){
                ...
                List<TradingPriceTracking> priceTrackings=iTradingPriceTracking.selectPriceTrackingByItemId(itemid);
                TradingPriceTracking priceTracking=new TradingPriceTracking();
                if(priceTrackings==null||priceTrackings.size()==0){
                    priceTracking.setItemid(itemid);
                    priceTracking.setUpdatecount(0L);
                    iTradingPriceTracking.savePriceTracking(priceTracking);
                }else{
                    priceTracking=priceTrackings.get(0);
                }
                ...
                rule.setPricetrackingId(priceTracking.getId());
    ...
    public void deleteAutoPricing(CommonParmVO commonParmVO,HttpServletRequest request) throws Exception {
        ...
        if(autoPricing!=null){
            ...
            if(rules!=null&&rules.size()>0){
                for(TradingPriceTrackingPricingRule rule:rules){
                    iTradingPriceTracking.removePriceTracking(rule.getPricetrackingId());
