...
public class GetOrdersController extends BaseAction {
    ...
    @Autowired
    private  ITradingListingData listingData;
    @Autowired
    ...
    @Autowired
    private ITradingOrderRefundSuccess iTradingOrderRefundSuccess;
    @Autowired
   ...
   /* SystemLogUtils
    ...
    public ModelAndView OrdersList(HttpServletRequest request,HttpServletResponse response,@ModelAttribute( "initSomeParmMap" )ModelMap modelMap){
        ...
        String onFlag = request.getParameter("onFlag");
        if(onFlag!=null){
            modelMap.put("onFlag",onFlag);
        }
    ...
    public void removeOrderTabremark(HttpServletRequest request,HttpServletResponse response,ModelMap modelMap) throws Exception {
       ...
       /* SystemLog systemLog=new SystemLog();
        ...
        if(config!=null){
            ...
            List<Long> ebayNames=new ArrayList<Long>();
            for(UsercontrollerEbayAccountExtend ebay:ebays){
                ebayNames.add(ebay.getId());
            }
            ...
            /*systemLog.setEventdesc("删除文件夹("+configName+")成功");
            SystemLogUtils.saveLog(systemLog);*/
            AjaxSupport.sendSuccessText("", "删除文件夹成功");
            return;
        }else{
            /*systemLog.setEventdesc("删除文件夹失败,文件夹不存在");
            SystemLogUtils.saveLog(systemLog);*/
            AjaxSupport.sendFailText("fail","文件夹不存在");
        }
    ...
    public void deleteOrder(HttpServletRequest request,HttpServletResponse response,ModelMap modelMap) throws Exception {
        ...
        systemLog.setOperuser(sessionVO.getUserName());
        if(StringUtils.isNotBlank(id)){
            TradingOrderGetOrders order1=iTradingOrderGetOrders.selectOrderGetOrdersById(Long.valueOf(id));
            List<TradingOrderGetOrders> orders=iTradingOrderGetOrders.selectOrderGetOrdersByOrderId(order1.getOrderid());
            for(TradingOrderGetOrders order:orders){
                iTradingOrderGetOrders.deleteOrderGetOrders(order.getId());
            }
            systemLog.setEventdesc("订单删除成功,订单号:"+orders.get(0).getOrderid());
            systemLog.setTableId(Long.valueOf(id));
            SystemLogUtils.saveLog(systemLog);
            AjaxSupport.sendSuccessText("","删除成功");
        }else{
            systemLog.setEventdesc("订单删除失败,订单不存在,id:"+id);
            SystemLogUtils.saveLog(systemLog);
    ...
    public void saveFolder(HttpServletRequest request,HttpServletResponse response,ModelMap modelMap) throws Exception {
        ...
        if(i==-2){
            systemLog.setEventdesc("移动文件夹失败,文件夹不存在");
            SystemLogUtils.saveLog(systemLog);
            AjaxSupport.sendFailText("fail","文件夹不存在");
            return;
        }
    ...
    public void saveComment(HttpServletRequest request,HttpServletResponse response,ModelMap modelMap) throws Exception {
        ...
        if(StringUtils.isNotBlank(orderid)){
            ...
            for(TradingOrderGetOrders order:orders){
                ...
                systemLog1.setEventname(SystemLogUtils.ORDER_OPERATE_RECORD);
                systemLog1.setOperuser(sessionVO.getUserName());
                order.setComment(comment);
                systemLog1.setEventdesc("保存备注成功,订单号:"+order.getOrderid()+",备注内容:"+comment);
                systemLog1.setTableId(order.getId());
                iTradingOrderGetOrders.saveOrderGetOrders(order);
                SystemLogUtils.saveLog(systemLog1);
            }
            AjaxSupport.sendSuccessText("","添加备注成功");
        }else{
            systemLog.setEventdesc("保存备注失败,订单不存在,订单号:"+orderid+",备注内容:"+comment);
            SystemLogUtils.saveLog(systemLog);
    ...
    public void saveTabremark(HttpServletRequest request,HttpServletResponse response,ModelMap modelMap) throws Exception {
        ...
        String folderType=request.getParameter("folderType");
        Asserts.assertTrue(StringUtils.isNotBlank(tabName),"文件夹名称不能为空");
        tabName=StringUtils.deleteWhitespace(tabName);
        Asserts.assertTrue(StringUtils.isAlphanumeric(tabName),"文件夹名称只能由数字和字母组成！");
        Asserts.assertTrue(StringUtils.isNotBlank(folderType),"文件夹类型不能为空！请重试");
        SessionVO sessionVO= SessionCacheSupport.getSessionVO();
        ...
        iPublicUserConfig.saveUserConfig(config);
       // systemLog.setEventdesc("保存文件夹成功，文件夹名字:"+tabName);
        //SystemLogUtils.saveLog(systemLog);
    ...
    @ResponseBody
    public void loadOrdersList(CommonParmVO commonParmVO,
        ...
        if(StringUtils.isBlank(accountQ)){
            accountQ=null;
        }
        ...
        if(StringUtils.isBlank(itemType)||"选择类型".equalsIgnoreCase(itemType)){
            itemType=null;
        }
        if(StringUtils.isBlank(folderId)){
            folderId=null;
        }
        if(StringUtils.isBlank(framConten)){
            framConten=null;
        }
        if(starttime!=null){
            starttime=DateUtils.turnToDateStart(starttime);
        }
        if(endtime!=null){
            endtime=DateUtils.turnToDateEnd(endtime);
        }
        ...
        if(ebays.size()>0){
            ...
            for(OrderGetOrdersQuery list:lists){
                ...
                String sitiId="";
                String currency="";
                TradingListingData  listingD = listingData.selectByItemid(itemid);
                if (listingD==null || ObjectUtils.isLogicalNull(listingD.getSite())){
                    List<TradingOrderGetItem> itemList= iTradingOrderGetItem.selectOrderGetItemByItemId(itemid);
                    if (!ObjectUtils.isLogicalNull(itemList)){
                        sitiId=itemList.get(0).getSite();
                        currency=itemList.get(0).getCurrency();
                    }
                }else {
                    sitiId=listingD.getSite();
                }
                list.setItemSite(sitiId);
                if (StringUtils.isNotBlank(sitiId)){
                    List<TradingDataDictionary> dictionarys=iTradingDataDictionary.selectDictionaryByType("site", null, null, sitiId);
                    if(!ObjectUtils.isLogicalNull(dictionarys)){
                        if (StringUtils.isBlank(currency)){
                            currency = dictionarys.get(0).getValue1();
                        }
                        String root=request.getContextPath();
             ...
             /*   List<TradingOrderAddMemberMessageAAQToPartner> partners=iTradingOrderAddMemberMessageAAQToPartner.selectTradingOrderAddMemberMessageAAQToPartnerByTransactionId(list.getTransactionid(),3,list.getSelleruserid(),"true");
                if(partners!=null&&partners.size()>0){
                    list.setMessage(partners.get(0).getBody());
                }*/
                /*List<TradingOrderGetSellerTransactions> transactionses=iTradingOrderGetSellerTransactions.selectTradingOrderGetSellerTransactionsByTransactionId(list.getItemid(),list.getTransactionid());
                ...
                if(feedBackDetail!=null){
                    ...
                    list.setFeedbackMessageType(feedBackDetail.getCommenttype());
                }else{
    ...
    public ModelAndView GetOrder(@ModelAttribute( "initSomeParmMap" )ModelMap modelMap){
        ...
        for(UsercontrollerEbayAccountExtend ebay:ebays){
            ...
            if(taskGetOrderses!=null&&taskGetOrderses.size()>0){
                ebayDates.add(taskGetOrderses.get(taskGetOrderses.size()-1).getLastsyctime());
            }else{
                ebayDates.add(null);
            }
        }
                        ...
                        if(StringUtils.isNotBlank(transactionses.get(0).getExternaltransactionid())){
                            ...
                            try {
                                ...
                                if(acc==null){
                                    ...
                                    systemLog.setEventname(SystemLogUtils.ORDER_OPERATE_RECORD);
                                    systemLog.setOperuser(sessionVO.getUserName());
                                    systemLog.setEventdesc("paypal账号未验证,未验证的账号是:"+order.getSelleremail());
                                    systemLog.setTableId(order.getId());
                                    SystemLogUtils.saveLog(systemLog);
                                    logger.error("paypal账号未验证"+order.getSelleremail());
                                    palpayPrice="账号未验证";
                                ...
                                if(acc!=null&&StringUtils.isNotBlank(acc.getFeeAmount())){
                                    palpayPrice=acc.getFeeAmount();
                                }
                            }
                        }
    ...
    public ModelAndView viewTemplateInitTable(HttpServletRequest request,HttpServletResponse response,@ModelAttribute( "initSomeParmMap" )ModelMap modelMap) throws Exception {
        ...
        List<TradingOrderGetOrders> lists1=iTradingOrderGetOrders.selectOrderGetOrdersByTransactionId(transactionid,Long.valueOf(ebayId),lists.get(0).getItemid());
        Asserts.assertTrue(!ObjectUtils.isLogicalNull(lists1),"没有查询到数据，请稍后再试!");
        String palpayPrice="";
        ...
        if(lists!=null&&lists.size()>0){
            TradingOrderGetOrders td1 = lists.get(0);
            String itemId = td1.getItemid();
        ...
        List<List<TradingOrderOrderVariationSpecifics>> specificseLists=new ArrayList<>();
        for(TradingOrderGetOrders lll:lists){
            String variationSKU=lll.getVariationsku();
            Integer orderCount=0;
            if(StringUtils.isNotBlank(variationSKU)) {
                List<TradingOrderOrderVariationSpecifics> specificses = iTradingOrderOrderVariationSpecifics.selectOrderOrderVariationSpecificsBySku(variationSKU);
                if(specificses!=null&&specificses.size()>0){
                    specificseLists.add(specificses);
                    orderCount=specificses.size();
                }else{
                    specificses=new ArrayList<>();
                    specificseLists.add(specificses);
                }
                orderCounts.add(orderCount);
            }
        }
        if(specificseLists!=null&&specificseLists.size()>0){
            modelMap.put("specificseLists",specificseLists);
            modelMap.put("orderCounts",orderCounts);
        }
        ...
        if(messageList!=null&&messageList.size()>0){
            for (TradingMessageGetmymessage messageGetmymessage: messageList){
                if ("false".equalsIgnoreCase(messageGetmymessage.getReplied())){
        ...
        List<TradingOrderAddMemberMessageAAQToPartner> addmessageList=iTradingOrderAddMemberMessageAAQToPartner.selectTradingOrderAddMemberMessageAAQToPartnerByItemIdAndSender(itemid,4,lists.get(0).getSelleruserid(),lists.get(0).getBuyeruserid(),"true");
        if(messageList!=null&&messageList.size()>0){
        ...
        modelMap.put("ebayId",lists1.get(0).getEbayId());
    ...
    public ModelAndView initIssueRefund(HttpServletRequest request,HttpServletResponse response,@ModelAttribute( "initSomeParmMap" )ModelMap modelMap) throws Exception {
        ...
        String ebayId=request.getParameter("ebayId");
        String orderId = request.getParameter("orderId");
        List<TradingOrderGetOrders> liorders= this.iTradingOrderGetOrders.selectOrderGetOrdersByOrderId(orderId);
        List<TradingOrderGetOrders> orders=iTradingOrderGetOrders.selectOrderGetOrdersByTransactionId(transactionid,Long.valueOf(ebayId),liorders.get(0).getItemid());
        List<TradingOrderRefundSuccess> lists=new ArrayList<>();
        if(orders!=null&&orders.size()>0){
           lists=iTradingOrderRefundSuccess.selectTradingOrderRefundSuccessByOrderId(orders.get(0).getOrderid());

            List<TradingOrderGetItem> itemList= iTradingOrderGetItem.selectOrderGetItemByItemId(orders.get(0).getItemid());
            if (!ObjectUtils.isLogicalNull(itemList)){
                modelMap.put("nowurrency",itemList.get(0).getCurrency());
            }

            List<TradingDataDictionary> dictionarys=iTradingDataDictionary.selectDictionaryByType("site", null, null, null);
            dictionarys=MyCollectionsUtil.listUnique(dictionarys,"value1");
            modelMap.put("currencys",dictionarys);
        }
        modelMap.put("refunds",lists);
    ...
    @AvoidDuplicateSubmission(needSaveToken = true)
    public ModelAndView modifyOrderStatus(Long primId
            ,@ModelAttribute( "initSomeParmMap" )ModelMap modelMap) throws Exception {

        TradingOrderGetOrders order = iTradingOrderGetOrders.selectOrderGetOrdersById(primId);
        modelMap.put("order",order);
       /* String transactionid=request.getParameter("transactionid");
        String ebayId=request.getParameter("ebayId");
        String orderId = request.getParameter("orderId");*/

        /*List<TradingOrderGetOrders> orderli  = this.iTradingOrderGetOrders.selectOrderGetOrdersByOrderId(orderId);
        List<TradingOrderGetOrders> orderList=iTradingOrderGetOrders.selectOrderGetOrdersByTransactionId(transactionid,Long.valueOf(ebayId),orderli.get(0).getItemid());
    ...
    */
    ...
    @AvoidDuplicateSubmission(needRemoveToken = true)
    @ResponseBody
    public void sendRefund(String refund,
        ...
        String code = (String)request.getSession().getAttribute("order_Refund_Captcha_Str");
        Asserts.assertTrue(StringUtils.isNotBlank(code)&&StringUtils.isNotBlank(valiStr),"验证码不能为空！");
        String itemId = request.getParameter("itemId");
        if (!code.equalsIgnoreCase(valiStr)){
            request.getSession().setAttribute("order_Refund_Captcha_Str", "&%^%^%^HGHJ_tembin_+_--");
            Asserts.assertTrue(false,"验证码不正确或已失效！请重新获取验证码！");
        }


        Asserts.assertTrue(StringUtils.isNotBlank(refund),"退款选项不能为空!");
        Asserts.assertTrue(StringUtils.isNotBlank(transactionid) && StringUtils.isNotBlank(ebayId),"必要参数不能为空");
        SessionVO sessionVO= SessionCacheSupport.getSessionVO();
        Asserts.assertTrue(sessionVO!=null,"系统错误不能获取session");
        SystemLog systemLog=new SystemLog();
        systemLog.setEventname(SystemLogUtils.ORDER_OPERATE_RECORD_REFUND);
        systemLog.setOperuser(sessionVO.getUserName());
        ...
        Asserts.assertTrue(!ObjectUtils.isLogicalNull(orderses),"没有找到订单!");
        Asserts.assertTrue("Complete".equals(orderses.get(0).getStatus()),"订单未付款!");
        List<TradingOrderGetSellerTransactions> lists=new ArrayList<>();
        if(orderses!=null&&orderses.size()>0&&orderses.get(0).getItemid()!=null){
            lists=iTradingOrderGetSellerTransactions.selectTradingOrderGetSellerTransactionsByTransactionId(orderses.get(0).getItemid(),transactionid);
        }
        Asserts.assertTrue(!ObjectUtils.isLogicalNull(lists),"PayPal交易号没找到");
        Map map=new HashMap();
        UsercontrollerEbayAccount u= iUsercontrollerEbayAccount.selectById(Long.valueOf(ebayId));
        Asserts.assertTrue(u!=null,"没有找到ebay帐号信息");
        String TID=lists.get(0).getExternaltransactionid();
        if("1".equalsIgnoreCase(refund)){
            map.put("transactionID",TID);
            map.put("paypalId",u.getPaypalAccountId());
            map1=payPalService.refundTransactionFull(map);
        }else if("2".equalsIgnoreCase(refund)){
            map.put("transactionID",TID);
            map.put("paypalId", u.getPaypalAccountId());
            Asserts.assertTrue(StringUtils.isNotBlank(amout), "部分退款，金额不能为空！");
            String itemid=lists.get(0).getItemid();

            if (StringUtils.isBlank(currency)){//如果参数是空，那么就查询
                List<TradingOrderGetItem> itemList= iTradingOrderGetItem.selectOrderGetItemByItemId(itemid);
                currency="USD";
                if (!ObjectUtils.isLogicalNull(itemList)){
                    currency=itemList.get(0).getCurrency();
        ...
        if(map1==null){
            systemLog.setEventdesc("退款操作失败交易号"+TID);
            systemLog.setTableId(orderses.get(0).getId());
            SystemLogUtils.saveLog(systemLog);
            ...
            systemLog.setEventdesc(hua+"交易号:"+TID);
            systemLog.setTableId(orderses.get(0).getId());
            SystemLogUtils.saveLog(systemLog);
            if(orderses!=null&&orderses.size()>0){
                TradingOrderRefundSuccess refundSuccess=new TradingOrderRefundSuccess();
                refundSuccess.setOrderid(orderses.get(0).getOrderid());
                Date date=new Date();
                String desc=sessionVO.getUserName()+"在"+date.toString()+"退款成功";
                if("1".equalsIgnoreCase(refund)){
                    desc=desc+",全款退还!";
                }else if("2".equalsIgnoreCase(refund)){
                    desc=desc+",金额为:"+amout+"USD!";
                }
                refundSuccess.setRefunddesc(desc);
                iTradingOrderRefundSuccess.saveTradingOrderRefundSuccess(refundSuccess);
            }
            AjaxSupport.sendSuccessText("message", "退款成功!");
        }else if(map1.get("flag")!=null&&"Unverified".equals(map1.get("flag"))){
            systemLog.setEventdesc("paypal账号未验证,交易号"+TID);
            systemLog.setTableId(orderses.get(0).getId());
            SystemLogUtils.saveLog(systemLog);
            AjaxSupport.sendFailText("fail", "退款失败,paypal账号未验证!");
        }else if(map1.get("flag")!=null&&"Failure".equals(map1.get("flag"))){
            systemLog.setEventdesc("退款操作失败,原因:"+map1.get("message")+",交易号"+TID);
            systemLog.setTableId(orderses.get(0).getId());
            SystemLogUtils.saveLog(systemLog);
            logger.error("订单退款失败:"+map1.get("res"));
            AjaxSupport.sendFailText("fail", map1.get("message"));
        }else{
            systemLog.setEventdesc("退款操作失败,交易号"+TID);
            systemLog.setTableId(orderses.get(0).getId());
            SystemLogUtils.saveLog(systemLog);
            AjaxSupport.sendFailText("fail", "退款失败,请稍后重试!");
        }
    ...
    public void apiModifyOrdersMessage(CommonParmVO commonParmVO,HttpServletRequest request) throws Exception {
        ...
        systemLog.setOperuser(sessionVO.getUserName());
        String res="";
        String ShipmentTrackingNumber1="";
        while (i>=0) {
            ...
            if(StringUtils.isNotBlank(transactionid)) {
                ...
                String ebayId = request.getParameter("ebayId" + i);
                Map map1=new HashMap();
                ...
                if ("fail".equalsIgnoreCase(r1)) {
                    i = -1;
                    ShipmentTrackingNumber1=ShipmentTrackingNumber;
                    logger.error("订单中上传跟踪号("+ShipmentTrackingNumber+")失败:"+res);
                }
                String ack = SamplePaseXml.getVFromXmlString(res, "Ack");
                if ("Success".equalsIgnoreCase(ack)) {
                    if (StringUtils.isNotBlank(ebayId)){
                        List<TradingOrderGetOrders> orderList = iTradingOrderGetOrders.selectOrderGetOrdersByTransactionId(transactionid, Long.valueOf(ebayId),itemid);
                        ...
                        systemLog1.setEventname(SystemLogUtils.ORDER_OPERATE_RECORD);
                        systemLog1.setOperuser(sessionVO.getUserName());
                        systemLog1.setEventdesc("批量上传跟踪号("+ShipmentTrackingNumber+")成功,订单号:"+order.getOrderid());
                        systemLog1.setTableId(order.getId());
                        SystemLogUtils.saveLog(systemLog1);
                    }
                    i++;
                    ...
                    i = -1;
                    ShipmentTrackingNumber1=ShipmentTrackingNumber;
                    logger.error("订单中上传跟踪号失败:"+sessionVO.getLoginId()+">>"+ShipmentTrackingNumber1+"::::"+res);
                }
        ...
        if(i==-1){
            systemLog.setEventdesc("批量上传跟踪号("+ShipmentTrackingNumber1+")失败:调用API错误:"+SamplePaseXml.getWarningInformation(res));
            SystemLogUtils.saveLog(systemLog);
    ...
    public ModelAndView viewOrderAbstractLeft(HttpServletRequest request,HttpServletResponse response,@ModelAttribute( "initSomeParmMap" )ModelMap modelMap) throws Exception {
        String TransactionId=request.getParameter("TransactionId");
        String ebayId=request.getParameter("ebayId");
        // todo 在界面中查找请求中都为注释掉的
        List<TradingOrderGetOrders> lists=iTradingOrderGetOrders.selectOrderGetOrdersByTransactionId(TransactionId,Long.valueOf(ebayId),"");
...
*/
        List<TradingOrderGetSellerTransactions> sellerTransactions=iTradingOrderGetSellerTransactions.selectTradingOrderGetSellerTransactionsByTransactionId(lists.get(0).getItemid(),lists.get(0).getTransactionid());
        String transactionid="";
        if(sellerTransactions!=null&&sellerTransactions.size()>0){
    ...
    public ModelAndView viewOrderShipmentsHistory(HttpServletRequest request,HttpServletResponse response,@ModelAttribute( "initSomeParmMap" )ModelMap modelMap) throws Exception {
        String TransactionId=request.getParameter("TransactionId");
        String ebayId=request.getParameter("ebayId");
        // todo 在界面请求中，未发现可用的请求
        List<TradingOrderGetOrders> lists=iTradingOrderGetOrders.selectOrderGetOrdersByTransactionId(TransactionId,Long.valueOf(ebayId),"");
        List<TradingOrderAddMemberMessageAAQToPartner> addMes=new ArrayList<TradingOrderAddMemberMessageAAQToPartner>();
        for(TradingOrderGetOrders order:lists){
            List<TradingOrderAddMemberMessageAAQToPartner> addmessageList1=iTradingOrderAddMemberMessageAAQToPartner.selectTradingOrderAddMemberMessageAAQToPartnerByTransactionId(order.getTransactionid(),order.getItemid(),1,order.getEbayId(),"true");
    ...
    public ModelAndView initOrdersSendMessage(HttpServletRequest request,HttpServletResponse response,ModelMap modelMap){
        ...
        String ebayId=request.getParameter("ebayId");
        String orderId = request.getParameter("orderId");
        List<TradingOrderGetOrders> orli = this.iTradingOrderGetOrders.selectOrderGetOrdersByOrderId(orderId);
        List<TradingOrderGetOrders> list=iTradingOrderGetOrders.selectOrderGetOrdersByTransactionId(transactionid,Long.valueOf(ebayId),orli.get(0).getItemid());
    ...
    */
    ...
    public ModelAndView initSendEvaluateMessage(HttpServletRequest request,HttpServletResponse response,ModelMap modelMap){
        ...
        String ebayId=request.getParameter("ebayId");
        String itemId = request.getParameter("itemId");
        List<TradingOrderGetOrders> list=iTradingOrderGetOrders.selectOrderGetOrdersByTransactionId(transactionid,Long.valueOf(ebayId),itemId);
        modelMap.put("order",list.get(0));
   ...
   */
    ...
    public void  downloadOrders(HttpServletRequest request,HttpServletResponse response,Date endtime,Date starttime) throws Exception {
        ...
        List<OrderGetOrdersQuery> orders=new ArrayList<OrderGetOrdersQuery>();
        if(ebays.size()>0) {
            if (ebays != null && ebays.size() > 0) {
                map.put("ebays", ebays);
            }
            ...
            orders = this.iTradingOrderGetOrders.selectOrderGetOrdersByGroupList(map, page);
        }
        ...
        systemLog.setEventname(SystemLogUtils.ORDER_OPERATE_RECORD);
        systemLog.setOperuser(sessionVO.getUserName());
        systemLog.setEventdesc("订单下载成功"+sessionVO.getLoginId());
        SystemLogUtils.saveLog(systemLog);
        iTradingOrderGetOrders.downloadOrders(orders, outputFile,response);
    }
     /*
     *订单中卖家发消息
     */
    ...
    @AvoidDuplicateSubmission(needRemoveToken = true)
    @ResponseBody
    public void apiGetOrdersSendMessage(HttpServletRequest request,
                                        String id,
                                        String body,
                                        String subject,
                                        String itemid,
        ...
        if(!StringUtils.isNotBlank(ebayId)){
            ebayId=request.getParameter("ebayId1");
        }
        ...
        if (StringUtils.isBlank(ebayName)){
            ebayName=selleruserid;
        }
        Map map1=new HashMap();
        map1.put("needToken","1");
        List<UsercontrollerEbayAccountExtend> dList= systemUserManagerService.queryCurrAllEbay(map1);
        ...
        Asserts.assertTrue(StringUtils.isNotBlank(token),"没获取到["+ebayName+"]的token");
        ...
        */
        String xml="";
        if((StringUtils.isNotBlank(transactionid) && StringUtils.isBlank(parentMessageID))
                ||"sellerSend".equalsIgnoreCase(msource)){
            ...
            map.put("itemid",itemid);
            map.put("buyeruserid", buyeruserid);
            xml = BindAccountAPI.getAddMemberMessageAAQToPartner(map);//获取接受消息
            AddApiTask addApiTask = new AddApiTask();
            // resMap = addApiTask.exec(d, xml, "https://api.ebay.com/ws/api.dll");
            resMap = addApiTask.exec(d, xml, apiUrl);
        }else if(StringUtils.isNotBlank(messageID) && StringUtils.isNotBlank(parentMessageID)){
            ...
            xml = BindAccountAPI.getAddMemberMessageRTQ(tradingMessageAddmembermessage,token);//获取接受消息
            AddApiTask addApiTask = new AddApiTask();
            //resMap = addApiTask.exec(d, xml, "https://api.ebay.com/ws/api.dll");
            resMap = addApiTask.exec(d, xml, apiUrl);
        }else{
            AjaxSupport.sendFailText("fail", "不能发送消息,无订单和收件箱!");
            return;
        }
        String r1 = resMap.get("stat");
        String res = resMap.get("message");
        try{
            if ("fail".equalsIgnoreCase(r1)) {
                ...
                message1.setFailereason(SamplePaseXml.getWarningInformation(res));
                message1.setEbayId(Long.valueOf(ebayId));
                iTradingOrderAddMemberMessageAAQToPartner.saveOrderAddMemberMessageAAQToPartner(message1);
                systemLog.setEventdesc("发送消息失败:调用API失败:"+SamplePaseXml.getWarningInformation(res));
                if(StringUtils.isNotBlank(id)){
                    systemLog.setTableId(Long.valueOf(id));
                    SystemLogUtils.saveLog(systemLog);
                }
            ...
            if ("Success".equalsIgnoreCase(ack)||"Warning".equalsIgnoreCase(ack)) {
                ...
                if(StringUtils.isNotBlank(messageID)&&StringUtils.isNotBlank(buyeruserid)){
                    List<TradingMessageGetmymessage> messageGetmymessage=iTradingMessageGetmymessage.selectMessageGetmymessageByMessageId(messageID,Long.valueOf(ebayId));
                ...
                if(StringUtils.isNotBlank(messageID)){
                    List<TradingOrderAddMemberMessageAAQToPartner> partners=iTradingOrderAddMemberMessageAAQToPartner.selectTradingOrderAddMemberMessageAAQToPartnerByMessageID(messageID,4,"false");
                    if(partners!=null&&partners.size()>0){
                        for(TradingOrderAddMemberMessageAAQToPartner partner:partners){
                            iTradingOrderAddMemberMessageAAQToPartner.deleteTradingOrderAddMemberMessageAAQToPartner(partner);
                        }
                    }
                }
                if(StringUtils.isNotBlank(transactionid)&&StringUtils.isNotBlank(sender)){
                    List<TradingOrderAddMemberMessageAAQToPartner> partners=iTradingOrderAddMemberMessageAAQToPartner.selectTradingOrderAddMemberMessageAAQToPartnerByTransactionId(transactionid, itemid,4, Long.valueOf(ebayId), "false");
                    if(partners!=null&&partners.size()>0){
                        for(TradingOrderAddMemberMessageAAQToPartner partner:partners){
                            iTradingOrderAddMemberMessageAAQToPartner.deleteTradingOrderAddMemberMessageAAQToPartner(partner);
                        }
    ...
    */
    ...
    public void GetOrderTransactions(CommonParmVO commonParmVO,HttpServletRequest request) throws Exception {
        ...
        Asserts.assertTrue(orderIds.length==1,"一次请同步一个订单！");
        /*Map map1=new HashMap();
        List<UsercontrollerEbayAccountExtend> list=systemUserManagerService.queryCurrAllEbay(map1);*/
        String token="";
        List<TradingOrderGetOrders> oos= iTradingOrderGetOrders.selectOrderGetOrdersByOrderId(orderIds[0]);
        Asserts.assertTrue(!ObjectUtils.isLogicalNull(oos),"数据中没有该订单，不能同步！");

        //测试环境
        token=userInfoService.getTokenByEbayID(oos.get(0).getEbayId());
    ...
    public void apiGetOrdersRequest(CommonParmVO commonParmVO,HttpServletRequest request) throws Exception {
        ...
        for(Long ebay:ebays){
            if(taskGetOrderses!=null&&taskGetOrderses.size()>0){
                Long ebayNameID=taskGetOrderses.get(0).getEbayid();
                ...
                if(ebay1.getId()!=(ebayNameID)){
                    ...
                    for(TaskGetOrders taskGetOrders:taskGetOrderses){
                        if(ebay1.getId()==(taskGetOrders.getEbayid())){
                ...
                for(UsercontrollerEbayAccount ebayAcount:ebayAcounts){
                    ...
                    taskGetOrders.setToken(ebayAcount.getEbayToken());
                    taskGetOrders.setTokenflag(0);
                    taskGetOrders.setFromtime(start);
                    taskGetOrders.setTotime(end);
                    taskGetOrders.setSavetime(date);
                    taskGetOrders.setUserid(ebayAcount.getUserId());
                    taskGetOrders.setEbayname(ebayAcount.getEbayName());
                    taskGetOrders.setEbayid(ebayAcount.getId());
                    Date createDate1=ebayAcount.getCreateTime();
                    ...
                    List<TradingOrderGetOrders> orders=iTradingOrderGetOrders.selectOrderGetOrdersByCreatedDateAndEbayAcount(createDate,ebayAcount.getEbayName());
                    if(orders!=null&&orders.size()==0){
                        ...
                        taskGetOrders1.setEbayid(ebayAcount.getId());
