...
import com.common.interceptor.randr.SessionVOInterceptor;
...
public class StringFilter implements Filter {
    private static List<String> excludePaths=new ArrayList<String>();//Referer必须在这个list之中，防止被嵌套
    static {
    ...
    //一些不需要验证refer的地址，url如果包含则返回true
    public static boolean isInReferWhiteBill(String url){
        List<String> wurlList=new ArrayList<String>();
        wurlList.add("countSome.do");//统计注册用户
        wurlList.add("redirectAfterAuthorize.do");//速卖通授权转向
        wurlList.add("saveTemplateInitTable.do");
        wurlList.add("pictureList.do");
        wurlList.add("sysUserManPage.do");
        wurlList.add("InventoryList.do");//一些后台地址
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        ...
        if("image/webp,*/*;q=0.8".equalsIgnoreCase(headerAccept)&&requestUrl.endsWith(".do")
                &&!requestUrl.endsWith("captchaActionlogin.do")
                &&!requestUrl.endsWith("orderRefundCaptchaAction.do")){//要排除掉两个验证码的请求
            return;
        }
        if (SessionVOInterceptor.isInWhiteBill(requestUrl) || isInReferWhiteBill(requestUrl)){
        ...
        if (requestUrl.endsWith("upLoadImage.do")){
            boolean b =ServletFileUpload.isMultipartContent(request);//CommonsMultipartResolver isMultipart
            if (!b){
                return;
            }
