    ...
    public void afterCompletion(HttpServletRequest request,
        ...
        if(ex!=null){
            ...
            if(DataAccessException.class.isAssignableFrom(ex.getClass())){//如果是数据库访问的出错
                m="数据库查询出错了！请联系客服。";
            }else if (ex instanceof RedirectAssertsException){ //如果是断言异常，不处理
                /*request.setAttribute("errmessage",((RedirectAssertsException) ex).getMessage());
                response.sendRedirect(request.getContextPath()+"/errorPage.jsp?c=2");*/
