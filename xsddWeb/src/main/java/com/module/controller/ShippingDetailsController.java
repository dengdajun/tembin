...
public class ShippingDetailsController extends BaseAction{
    ...
    public void loadShippingDetailsList(HttpServletRequest request,ModelMap modelMap,CommonParmVO commonParmVO) throws Exception {
      ...
      /*  List<UsercontrollerUserExtend> liuue = systemUserManagerService.queryAllUsersByOrgID("yes");
        ...
        for(ShippingdetailsQuery l:lisq){
            if(systemUserManagerService.isMainUserAcount()){
    ...
    public void shipingService(ModelMap modelMap,HttpServletRequest request){
        ...
        for (Map<String,List<TradingDataDictionary>> ma : maps){
            for (Map.Entry e : ma.entrySet()){
                ...
                for(TradingDataDictionary tdd:f){
                    if(tdd.getName().indexOf("至")==-1) {
                        if (!ObjectUtils.isLogicalNull(tdd.getTimermax()) && !ObjectUtils.isLogicalNull(tdd.getTimermin()) && !tdd.getTimermax().equals(tdd.getTimermin())) {
                            tdd.setName(tdd.getName() + "[" + tdd.getTimermin() + " 至 " + tdd.getTimermax() + " 工作日]");
                        }
                    }
                }
                d.setDictionaries(f);
     ...
     * @return
     */
    @RequestMapping("/ajax/shipingInterService.do")
    @ResponseBody
    public void shipingInterService(ModelMap modelMap,HttpServletRequest request) {
        List<TradingDataDictionary> litype = DataDictionarySupport.getTradingDataDictionaryByType(DataDictionarySupport.DATA_DICT_SHIPPINGINTER_TYPE,Long.parseLong(request.getParameter("parentID")));
        List<TradingDataDictionary> liss = new ArrayList<>();
        for(TradingDataDictionary tdd:litype){
            if(tdd.getName().indexOf("至")==-1) {
                if (!ObjectUtils.isLogicalNull(tdd.getTimermax()) && !ObjectUtils.isLogicalNull(tdd.getTimermin()) && !tdd.getTimermax().equals(tdd.getTimermin())) {
                    tdd.setName(tdd.getName() + "[" + tdd.getTimermin() + " 至 " + tdd.getTimermax() + " 工作日]");
        ...
        AjaxSupport.sendSuccessText("",liss);
    }
...
*/
    ...
    public void saveShippingDetails(HttpServletRequest request,TradingShippingdetails tradingShippingdetails,ShippingDetails shippingDetails) throws Exception {
        List<InternationalShippingServiceOption> liin = shippingDetails.getInternationalShippingServiceOption();
        if(liin!=null&&liin.size()>0){
            for(InternationalShippingServiceOption inter : liin){
                if(inter.getShipToLocation()==null||inter.getShipToLocation().size()==0){
                    Asserts.assertTrue(false,"国际运输到地方未选择！");
                    AjaxSupport.sendFailText("","国际运输到地方未选择！");
                    return;
                }
            }
        }
     ...
     * @return
     */
    @RequestMapping("/ajax/ShippingLocationto.do")
    @ResponseBody
    public void ShippingLocationto(HttpServletRequest request,HttpServletResponse response,ModelMap modelMap){
        String parentid = request.getParameter("parentID");
        List<TradingDataDictionary> lidata = DataDictionarySupport.getTradingDataDictionaryByType(DataDictionarySupport.SHIPPINGTOADDRESS,Long.parseLong(parentid));
        AjaxSupport.sendSuccessText("",lidata);
    }
     ...
     * @param modelMap
     */
    @RequestMapping("/ajax/saveShippingdoc.do")
    @ResponseBody
    public void saveShippingdoc(HttpServletRequest request,HttpServletResponse response,ModelMap modelMap){
        String docid = request.getParameter("docid");
        String val = request.getParameter("val");
        String valname = request.getParameter("valname");
        String soureid = request.getParameter("soureid");
        if(valname.indexOf("interShipping")>0){//修改国际运输方式
            TradingInternationalshippingserviceoptionDoc tradingInternationalshippingserviceoptionDoc= this.iTradingInternationalShippingServiceOptionDoc.selectByDocidSourceId(Long.parseLong(docid),Long.parseLong(soureid));
            if(tradingInternationalshippingserviceoptionDoc==null){
                return;
            }
            if(valname.indexOf("interShippingServiceCost")>0){
                tradingInternationalshippingserviceoptionDoc.setShippingservicecost(Double.parseDouble(val));
            }else if(valname.indexOf("interShippingServiceAdditionalCost")>0){
                tradingInternationalshippingserviceoptionDoc.setShippingserviceadditionalcost(Double.parseDouble(val));
            }
            this.iTradingInternationalShippingServiceOptionDoc.svaeDoc(tradingInternationalshippingserviceoptionDoc);
        }else{//修改国内运输方式
            TradingShippingserviceoptionsDoc tradingShippingserviceoptionsDoc = this.iTradingShippingServiceOptionsDoc.selectByDocidSourceid(Long.parseLong(docid),Long.parseLong(soureid));
            if(tradingShippingserviceoptionsDoc==null)
            {
                return;
            }
            if(valname.indexOf("ShippingServiceCost")>0){
                tradingShippingserviceoptionsDoc.setShippingservicecost(Double.parseDouble(val));
            }else if(valname.indexOf("ShippingServiceAdditionalCost")>0){
                tradingShippingserviceoptionsDoc.setShippingserviceadditionalcost(Double.parseDouble(val));
            }else if(valname.indexOf("ShippingSurcharge")>0){
                tradingShippingserviceoptionsDoc.setShippingsurcharge(Double.parseDouble(val));
            }
            this.iTradingShippingServiceOptionsDoc.saveDoc(tradingShippingserviceoptionsDoc);
        }
