...
<head>
    ...
    <script type="text/javascript">
        ...
        function addAllOrder(){
            ...
            if(country[0].checked){
                var htm="<div id='selectAllOrder' style='margin-bottom: 5px;'>所有订单:<font style=\"color: #0000ff\">所有的订单</font><br/></div>"
                $("#addMarket").append($(htm));
            }else{
                    ...
                    <div>
                        ...
                        <div class="controls">
                            <select class="input-xlarge" name="type" style="float: left">
                                ...
                                <c:if test="${autoMessage.type=='paidAutoMessage'}">
                                    <option value="paidAutoMessage" selected>收到买家付款</option>
                                </c:if>
                                <c:if test="${autoMessage.type!='paidAutoMessage'}">
                                    <option value="paidAutoMessage">收到买家付款</option>
                                </c:if>
                                <c:if test="${autoMessage.type=='shippedAutoMessage'}">
                                    <option value="shippedAutoMessage" selected>标记已发货</option>
                                </c:if>
                                <c:if test="${autoMessage.type!='shippedAutoMessage'}">
                                    <option value="shippedAutoMessage">标记已发货</option>
                                </c:if>
                                <c:if test="${autoMessage.type=='goodReviewAutoMessage'}">
                                   <option value="goodReviewAutoMessage" selected>收到买家的正评</option>
                                </c:if>
                                <c:if test="${autoMessage.type!='goodReviewAutoMessage'}">
                                    <option value="goodReviewAutoMessage">收到买家的正评</option>
                                </c:if>
                                 <c:if test="${autoMessage.type=='mediumReviewAutoMessage'}">
                                   <option value="mediumReviewAutoMessage" selected>收到买家的中评</option>
                                 </c:if>
                                <c:if test="${autoMessage.type!='mediumReviewAutoMessage'}">
                                    <option value="mediumReviewAutoMessage">收到买家的中评</option>
                                </c:if>
                                <c:if test="${autoMessage.type=='badReviewAutoMessage'}">
                                    <option value="badReviewAutoMessage" selected>收到买家的负评</option>
                                </c:if>
                                <c:if test="${autoMessage.type!='badReviewAutoMessage'}">
                                    <option value="badReviewAutoMessage">收到买家的负评</option>
                                </c:if>
                            </select>&nbsp;后
                            <select name="day">
            ...
            <div style="width: 560px;height:320px;border: 1px #acd0f0 solid;float: left">
                ...
                <div style="padding: 20px;margin-top: 40px;" id="addMarket">
                    ...
                    <c:if test="${order!=null}">
                        <div id='selectAllOrder' style="margin-bottom: 5px;">所有订单:<font style="color: #0000ff">所有的订单</font><br/></div>
                    </c:if>
            ...
            <div style="width: 350px;height:320px;border: 1px #acd0f0 solid;float: right">
                ...
                <div id="regulation" style="padding: 20px;margin-top: 40px;">
                    <div  style="margin-bottom: 5px;">
                        <c:if test="${items[0]!=null}">
                        <input type="checkbox" name="item" checked onclick="addItem();">&nbsp;订单商品包含指定商品
                    </c:if>
                        <c:if test="${items[0]==null}">
                        <input type="checkbox" name="item"  onclick="addItem();">&nbsp;订单商品包含指定商品
                    </c:if>
                        </div>
                    <div  style="margin-bottom: 5px;">
                    <c:if test="${countrys[0]!=null}">
                        <input type="checkbox" name="country" checked onclick="addcountry();">&nbsp;订单目的地为指定国家
                    </c:if>
                    <c:if test="${countrys[0]==null}">
                        <input type="checkbox" name="country" onclick="addcountry();">&nbsp;订单目的地为指定国家
                    </c:if>
                        </div>
                    <div  style="margin-bottom: 5px;">
                    <c:if test="${amounts[0]!=null}">
                        <input type="checkbox" name="amount" checked onclick="addAmount();">&nbsp;订单来源为指定账号
                    </c:if>
                    <c:if test="${amounts[0]==null}">
                        <input type="checkbox" name="amount"  onclick="addAmount();">&nbsp;订单来源为指定账号
                    </c:if>
                        </div>

                    <div  style="margin-bottom: 5px;">
                    <c:if test="${services[0]!=null||internationalServices[0]!=null}">
                        <input type="checkbox" name="shippingService" checked onclick="addShippingService();">&nbsp;买家选择的物流为指定物流方式
                    </c:if>
                    <c:if test="${services[0]==null&&internationalServices[0]==null}">
                        <input type="checkbox" name="shippingService"  onclick="addShippingService();">&nbsp;买家选择的物流为指定物流方式
                    </c:if>
                        </div>
                        <div  style="margin-bottom: 5px;">
                    <c:if test="${exceptCountrys[0]!=null}">
                        <input type="checkbox" checked name="exceptCountry" onclick="addExceptCountry();">&nbsp;订单目的地为指定国家之外
                    </c:if>
                    <c:if test="${exceptCountrys[0]==null}">
                        <input type="checkbox" name="exceptCountry" onclick="addExceptCountry();">&nbsp;订单目的地为指定国家之外
                    </c:if>
                            </div>

                    <div  style="margin-bottom: 5px;">
                    <c:if test="${allEbay!=null}">
                        <input type="checkbox" checked name="allEbay" onclick="addAllEbay();">&nbsp;所有账户
                    </c:if>
                    <c:if test="${allEbay==null}">
                        <input type="checkbox" name="allEbay" onclick="addAllEbay();">&nbsp;所有账户
                    </c:if>
                        </div>

                    <div  style="margin-bottom: 5px;">
                    <c:if test="${order!=null}">
                        <input type="checkbox" checked name="allOrder" onclick="addAllOrder();">&nbsp;所有订单
                    </c:if>
                    <c:if test="${order==null}">
                        <input type="checkbox" id="allOrder1" name="allOrder" onclick="addAllOrder();">&nbsp;所有订单
                    </c:if>
                        </div>
                </div>
            </div>
