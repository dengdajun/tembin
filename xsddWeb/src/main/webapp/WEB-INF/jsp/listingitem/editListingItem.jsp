...
function loadListingItem(itemId,ebayAccount,siteid){
    ...
    $().invoke(
            ...
            [function (m, r) {
                ...
                if(myDescription==null||myDescription==undefined){
                    setTimeout(function(){
                ...
                var policyCount = "";
                var policyPeriod = "";
                if(item.buyerRequirementDetails.maximumBuyerPolicyViolations!=null){
                    policyCount = item.buyerRequirementDetails.maximumBuyerPolicyViolations.count;
                    policyPeriod = item.buyerRequirementDetails.maximumBuyerPolicyViolations.period;
                }
                ...
                var unpaidCount  = "";
                var unpaidPeriod = "";
                if(item.buyerRequirementDetails.maximumUnpaidItemStrikesInfo!=null){
                    unpaidCount  = item.buyerRequirementDetails.maximumUnpaidItemStrikesInfo.count;
                    unpaidPeriod = item.buyerRequirementDetails.maximumUnpaidItemStrikesInfo.period
                }
                ...
                if(item.shippingDetails!=null&&item.shippingDetails.shippingServiceOptions!=null){
                    ...
                    for(var i=0;i<item.shippingDetails.shippingServiceOptions.length;i++){
                        ...
                        $("#shippingMore").append(createTables(shi.shippingService,shi.shippingServiceCost.value,shi.freeShipping,shi.shippingServiceAdditionalCost.value,'0'));
                ...
                if(item.shippingDetails!=null&&item.shippingDetails.internationalShippingServiceOption!=null){
                    for(var i=0;i<item.shippingDetails.internationalShippingServiceOption.length;i++){
                        ...
                        $("select[name='ShippingService'][shortName='a2']").eq(i).find("option[atta='"+shi.shippingService+"']").attr("selected",true);
...
$(document).ready(function() {
    ...
    strss += '<li><div style="position:relative"><input type="hidden" name="blankimg" value="${temp.value}">' +
            '<img src="'+chuLiPotoUrl("${temp.value}")+'" height=\"80px\" width=\"78px\" />' +
...
function selectSite(obj){
    ...
    var urll = path+"/ajax/ShippingLocationto.do?parentID="+vals;
    $().invoke(
            urll,
            {},
            [function (m, r) {
                shippingLocation="";
                for(var i = 0;i< r.length;i++){
                    if(r[i].value=="Worldwide"){
                        continue;
                    }
                    shippingLocation+="<input type='checkbox' name='ShipToLocation' value='"+ r[i].value+"'> "+r[i].name;
                }
                $("div[name='shto']").html(shippingLocation);
            },
                function (m, r) {
                }]
    );
...
function setInter(parentId){
    var urll = path+"/ajax/shipingInterService.do?parentID="+parentId;
    var api = frameElement.api, W = api.opener;
    $().invoke(
            urll,
            {},
            [function (m, r) {
                interShippingService="";
                for(var i = 0;i< r.length;i++){
                    interShippingService +='<option value="'+r[i].id+'" atta="'+r[i].value+'">'+r[i].name+'</option>';
                }
                /*$("select[name='ShippingService'][shortName='a2']").each(function(i,d){
                    $(d).html(interShippingService);
                });*/
            },
                function (m, r) {
                    alert(r);
...
<div id="selectId" style="padding-left: 100px;padding-top: 20px;padding-bottom: 20px;">
    <table width="70%">
        ...
        <tr>
            <td><input type="checkbox" name="selectType" value="selectTemplate">刊登模板</td>
            <td>&nbsp;</td>
            <td>&nbsp;</td>
            <td>&nbsp;</td>
        </tr>
    ...
    <td colspan="2">
        <table>
            ...
            <tr>
                ...
                <td><input type="checkbox" name="isUpdateFlag" value="1" checked="true">是否更新范本</td>
            </tr>
        </table>
    </td>
</tr>
...
<tr id="ShippingDetails" style="display: none;border-bottom: 1px solid #e5e5e5;">
    ...
    <td>
        <table width="95%">
            ...
            <tr>
                <td colspan="2">
                    <span id="notLocationName" style="line-height: 32px;word-break:normal; width:650px; display:block; white-space:pre-wrap;word-wrap : break-word ;overflow: hidden ;"></span>
...
<tr id="selectTemplate" style="display: none;border-bottom: 1px solid #e5e5e5;">
    <td style="width: 20px;text-align: right;padding-right: 20px;">刊登模板</td>
    <td>
...
<script>
    ...
    function createInterTables(obj1,obj2,obj3){
        ...
        intertable +=' <td> ';
        intertable +=' <select name="ShippingService" style="width: 560px;"  shortName="a2"> ';
        if(interShippingService!=""){
            intertable +=interShippingService;
        }
        intertable +=' </select> ';
        intertable +=' </td> ';
        intertable +=' </tr> ';
        ...
        intertable +=' <td> ';
        intertable +=' <input type="checkbox" name="ShipToLocation"  value="Worldwide" onclick="selectLocation(this)"/> 全球 <a href="javascript:void(0)" onclick="selectAllLocation(this)">选择以下所有国家和地区</a>';
        intertable +=' </br>';
        intertable +=' <div name="shto">'+shippingLocation+'</div>';
        intertable +=' </td> ';
        intertable +=' </tr> ';
        intertable +=' </table> ';
        return intertable;
