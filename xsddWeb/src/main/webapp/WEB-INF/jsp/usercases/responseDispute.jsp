...
<head>
    ...
    <script type="text/javascript">
       ...
       var escalate;
       function escalateToCustomer(){
           if(!$("#escalateForm").validationEngine("validate")){
               return;
           }
           var escalateSellerid=$("#escalateSellerid").val();
           var escalateTransactionId=$("#escalateTransactionId").val();
           var escalateType=$("#escalateType").val();
           var textarea4=$("#textarea4").val();
           var ebayid=$("#ebayIdC").val();
           var url=path+'/userCases/escalateToCustomer.do?ebayId='+ebayid+'&sellerid='+escalateSellerid+"&transactionId="+escalateTransactionId+'&type='+escalateType+'&textarea='+textarea4;
           escalate = openMyDialog({title: 'ask ebay',
               content:'url:'+url,
               icon: 'succeed',
               width:700,
               height:350,
       ...
       $(document).ready(function(){
           ...
           $("#escalateForm").validationEngine();
       });
        function queryTrack(){
            var url=path+"/userCases/queryTrack.do?trackNum=${order.shipmenttrackingnumber}&transactionId=${order.transactionid}&seller=${order.selleruserid}&itemId=${order.itemid}";
            $().invoke(url,null,
                    [function(m,r){
                        /*W.CaseDetails.close();*/
    ...
    function selectSendMessage(){
        ...
        content=content.replace("{eBay_Item_Title}","${caseTitle}");
    ...
    <td rowspan="3" valign="top" style="margin-left:20px; padding-left:15px; padding-top:20px; padding-right:20px; line-height:25px;background:#F4F4F4"><strong>订单号</strong><br>
       ...
       <%-- <c:forEach items="${items}" begin="0"  varStatus="status">
        ...
        <div style="font-family:Arial, Helvetica, sans-serif; font-size:12px;">
            ...
            <strong style="color:#000000">Case reason:</strong><br>
            ${createReason}
            <br/>
    ...
    <td width="772" valign="top"><div class="listbox_new jq_new">
        ...
        <div class="box" style="display: none;">
            <form id="sendTrackNum">
                <input type="hidden" id="ebayIdC" name="ebayId" value="${cases.ebayId}"/>
                <input type="hidden" name="transactionId" value="${cases.transactionid}"/>
                <input type="hidden" name="itemId" value="${cases.itemid}"/>
            <table width="100%" border="0">
                <tbody><tr>
        ...
        <h4><span>+</span>
            <table width="90%" border="0"><tbody><tr>
                <td><strong>Update tracking details</strong></td></tr></tbody></table>
        </h4>
        <div class="box" style="display: none;">
            <form id="sendWithoutNum">
                <input type="hidden" name="ebayId" value="${cases.ebayId}"/>
             <input type="hidden" name="transactionId" value="${cases.transactionid}"/>
            <table width="100%" border="0">
            <tbody>
            <tr><td height="32" align="left">You can add or edit the tracking details.</td></tr>
            <tr><td><hr></td></tr>
            <tr><td height="32" align="left">Add the tracking number:</td></tr>
            <tr>
                <td height="32" align="left"><input name="trackingNum" type="text" class="form-controlsd validate[required]" value=""></td>
            </tr>
            <tr><td height="32" align="left">Add the shipping carrier name:</td></tr>
            <tr>
                <td height="32" align="left"><input name="carrier" type="text" class="form-controlsd validate[required]" value=""></td>
            </tr>
            <tr>
                <td height="32" align="left">Comments</td>
            </tr>
            <tr>
                <td height="32" align="left">
                    <label for="textarea"></label>
        ...
        <div class="box" style="display: none;">
         ...
         <%--   <c:if test="${contentFlag=='true'}">
          ...
          <%--  <c:if test="${reasonFlag=='true'}">--%>
                <td height="32" align="left"><strong style="color: #000000">Refund amount ${cases.caseamount} ${currency}</strong></td>
          <%--  </c:if>--%>
            </tr>
           <%-- <c:if test="${contentFlag=='true'}">
                <tr>
        ...
        <div class="box" style="display: none;">
            <form id="escalateForm">
                <input type="hidden" id="escalateSellerid" name="ebayId" value="${cases.ebayId}"/>
                <input type="hidden" id="escalateTransactionId" name="transactionId" value="${cases.transactionid}"/>
            <table width="100%" border="0">
                <tbody>
                <tr><td height="32" align="left">it looks like it's time for us to step in and help</td></tr>
                <tr><td height="32" align="left">give us few more details,so we can help resolve the problem</td></tr>
                <tr><td height="32" align="left"><hr></td></tr>
                <tr><td height="32" align="left">tell us what the problem is:<br/></td></tr>
                <tr><td height="32" align="left"><select id="escalateType" class="form-controlsd" name="type">
                    <option value="">--Select a reason--</option>
                    <option value="ITEM_SHIPPED_WITH_TRACKING">I refunded the buyer,but the buyer isn't satisfied</option>
                    <option value="TROUBLE_COMMUNICATION_WITH_BUYER">I'm having trouble communicating with the buyer</option>
                    <option value="ITEM_SHIPPED_WITH_TRACKING">I shipped the item with tracking details</option>
                    <option value="OTHER">Other</option>
                </select><br/></td></tr>
                <tr><td height="32" align="left">Comments:<br/></td></tr>
                <tr>
                    <td height="32" align="left">
                        <label for="textarea"></label>
                        <textarea name="textarea4" id="textarea4" cols="100%" rows="5" class="newco validate[required]"><%--Additional comments:(optional)--%></textarea>
                    </td>
                </tr>
                </tbody>
            </table>
...
<tr>
    <td width="772">
        ...
        <button type="button" class="btn btn-default" onclick="closedialog12()" data-dismiss="modal">关闭</button>
        </div>
    </div> </td>
</tr>
</tbody></table>
