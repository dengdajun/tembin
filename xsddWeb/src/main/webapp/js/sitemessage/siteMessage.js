...
function queryMessageList(){
    $("#siteMessageDiv").initTable({
        ...
        columnData:[
            ...
            {title:"发件人",name:"messageFrom",width:"8%",align:"left",format:function(json){
                if(json.messageType=="意见反馈信息"){
                    return json.fromName;
                }else{
                    return "system";
                }
            }
...
function makeOptionM(json){
    ...
    if("意见反馈信息"==json.messageType){
        hs+="<li style='left:1px;' onclick=doSendMessage(this) value='"+json.id+"' doaction=\"look\" >回复</li>";
    }
    var pp={"liString":hs};
...
function doSendMessage(obj){
    var optionV=obj;//$(obj).find("option:selected");
    var id=$(optionV).attr('value');
    readMessageWindow=$.dialog({
        title:'回复',
        content:"url:"+path+"/sitemessage/initSendMessage.do?id="+id,
        width:850,
        height:600,
        max:false,
        min:false,
