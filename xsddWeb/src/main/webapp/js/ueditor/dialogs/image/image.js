                ...
                try{
                    if(parent.afterUploadCallback && parent.afterUploadCallback!=null){
                        ...
                        for(var i=0;i<list.length;i++){
                            var url = list[i].src;
                            if(url.indexOf("_small.")>0){
                                list[i].src=url.replace("_small.",".");
                                list[i]._src=url.replace("_small.",".");
                            }
                        }
                        f(list);
    ...
    RemoteImage.prototype = {
        ...
        getInsertList: function () {
            ...
            $("input[id^='url']").each(function(i,d) {
                var urllll=$(d).val();
                if(urllll!=null && urllll!='' && urllll.length>0){
                    var h=data['height'];
                    var w=data['width'];
                    if(w==null||w==''){
                        var imgdom=$(d.parentNode).find("img[id^=imgg]").eq(0);
                        h=imgdom.height();
                        w=imgdom.width();
                        if(w>800){
                            var jspx=w-800;//计算消减了多少
                            var _t=accDiv(jspx,w).toFixed(2);//计算消减的百分比
                            w=800;
                            h=accMul(h,_t).toFixed(0);
                        }

                    }
                    w=((w==null||w==0)?'':w);
                    h=((h==null||h==0)?'':h);
                    srcArr[i] = {src: urllll, _src: urllll,width: w,height: h};
                }
            });
            return srcArr;
/*//todo 在线图片可以选择多张
