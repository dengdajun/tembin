...
/**判断是否包含全角*/
function isContainQJ(s){
	return /[^\x00-\xff]/ig.test(s);
}
/**判断是否包含汉字*/
function isContainHZ(s){
	return /.*[\u4e00-\u9fa5]+.*$/.test(s);
}
...
function getTopWin_(){
	...
	{
		breakmark++;
		if(breakmark>5){breakmark=0 ; return obj;}
		try {
			obj.document.getElementById("contentMaindiv")
		} catch (e) {
...
/**通过图片的url来获取图片大小
function getImgHW(imgsrc,ind){
    var imgid="_imgtemp__"+ind;
    var res_={"w":"","h":""}
    $("<img/>").attr({"src": imgsrc,"id":imgid}).load(function(){
        res_.h=this.height;
        res_.w=this.width;
        $("#"+this.id).remove();
        return res_;
