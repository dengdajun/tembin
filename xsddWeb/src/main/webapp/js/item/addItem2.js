...
function clearThisText(obj) {
    if($(obj.parentNode)[0].cellIndex==2||$(obj.parentNode)[0].cellIndex==3){
        ...
        $("#moreAttrs tr td:nth-child(" + (indexs+ 1) + ")").each(function(i,d){
            if(($(d).find("input").val()==""||$(d).find("input").val()==null)&&val!=""){
...
function bodyClick(){
    ...
    $("#attTable").find("input[name='value']").prop("type","hidden");
    $("#attTable").find("input[name='value']").prop("type","hidden");
    $("#attTable").find("input[name='name']").prop("type","hidden");
    $("#attTable").find("span[name='name']").show();
    $("#attTable").find("span[name='value']").show();
    $("#attTable").find("span[name='values']").show();

    if($("#attTable").find("div[id^='s2id_']").length>0){
        $("#attTable").find("div[id^='s2id_']").hide();

        $("#attTable").find("div[id^='s2id_']").each(function(i,d){
            var nnid=$(d).siblings("input[class='multiSelect']").eq(0).attr("id");
            $(d).siblings("span[name='values']").html($("#"+nnid).select2("data").text);
        });
...
function removeROW(obj) {
    ...
    var attName=$(obj).parent().parent().find("td > input").eq(0).val();
...
function addc(obj) {
    ...
    if ($(obj.parentNode)[0].cellIndex == 4) {
        ...
        for (var i = 0; i < attrValue.keys.length; i++) {
            $("#picMore").append(addPic(attrName, attrValue.get(attrValue.keys[i])));
            $().image_editor.init(replaceTSFH(attrName+"this"+attrValue.get(attrValue.keys[i]))); //编辑器的实例id
            $().image_editor.show(replaceTSFH(attrValue.get(attrValue.keys[i]))); //上传图片的按钮id
        }
    }
    clearThisText(obj);
}
...
function addPic(attrName, attrValue) {
    ...
    $("#moreAttrs tr td:nth-child(5)").each(function (i, d) {
        if ($(d).find("input[name='attr_Value']").val() != undefined && $(d).find("input[name='attr_Value']").val() != "") {
            ...
            if($(d).find("input[name='attr_Value']").val()==attrValue||$(d).find("input[name='attr_Value']").val()==attrValue){
                vals=$(d).find("input[name='attr_Value']").val();
            }
        }
    });
    var str = "";
    str += "<div><div style='padding-top: 20px;'>" + attrName + ":" + vals + "</div> <section class='example' style='width: 1200px;'><ul class='gbin1-list' style='padding-left: 20px;' id='picturemore_"+replaceTSFH(attrValue)+"'></ul><div class='a_bal' style='margin-top: 20px;'></div></section> <script type=text/plain id='" + replaceTSFH(attrName) + "this" + replaceTSFH(attrValue) + "' />";
    str += "<div style='padding-left: 20px; '>" +
        "<b style='height: 32px;margin-top: 20px;' class='new_button'><a href='javascript:void(0)' id='" + replaceTSFH(attrValue) + "' bsid='"+attrValue+"' onClick='selectPic(this)'>选择图片</a></b>";
    str += "</div></div>";
    return str;
}
...
function selectPic(a) {
    ...
    sss = replaceTSFH(a.id);
    if (sss.indexOf("apicUrls")!=-1) {
        if (url.indexOf("editListingItem.do") != -1) {
            if (countChoosePic() > 11 ) {
                setTimeout(function () {
                    closeSelectPicWindow()
                }, 200);
        ...
        if (countChoosePic() > 12 * $("input[type='checkbox'][name='ebayAccounts']:checked").length) {
            setTimeout(function () {
                closeSelectPicWindow()
            }, 200);
            alert("上传的主图已超过12张，请删掉部份主图再上传多属性图片！");
            return;
        }
    }
...
function addPictrueUrl(urls) {
    if(ebayid==undefined||ebayid==""){
        ebayid = $("input[type='checkbox'][name='ebayAccounts']:checked").val();
    }
    ...
    if (sss.indexOf("apicUrls")!=-1) {//商品图片
        ...
        for (var i = 0; i < urls.length; i++) {
            ...
            str += '<li title="可拖拽排序"><div style="position:relative"><input type="hidden" name="pic_mackid"/> <input type="hidden" name="PictureDetails'+sss.substr(sss.indexOf("_"),sss.length)+'.PictureURL" value="' + imgsrc + '">' +
        ...
        var js = 0;
        for (var i = 0; i < urls.length; i++) {
            var idDuffs=generateMixedRandom(5);
            str+="<li><div style='position:relative'><img id=imgtemp"+idDuffs+" style='display:none;' src=" + chuLiPotoUrl(urls[i].src.replace("http@", "http:")) + " height='80px' width='78px' /><input type='hidden' name='pic_mackid_more'/><input type='hidden' name='" + sss + "' value='" + urls[i].src.replace("http@", "http:") + "'><img src='" + chuLiPotoUrl(urls[i].src.replace("http@", "http:")) + "' height='80' width='78' /><div style='text-align: right;background-color: dimgrey;'><img src='"+path+"/img/newpic_ico.png' onclick='removeThis(this)'></div></div></li>";
            urlss+=urls[i].src.replace("http@", "http:")+",";
            $("<img/>").attr({"src": (urls[i].src.replace("http@", "http:")),"id":"_imgtemp"+idDuffs}).load(function() {
                if((this.width>=500&&this.width<1000) || (this.height>=500&&this.height<1000)){
                    var targgetImgIndex=subRight(this.id,5)//strGetNum(this.id);
                    $("#imgtemp"+targgetImgIndex).wrap("<div style=\"border: solid red 1px;width: 80px;\"></div>");
                    afterTipFunction("imgtemp"+targgetImgIndex,"图像大小小于1000像素，可能会影响展示效果",true);
...
function getCategoryName(categoryId,siteId){
    ...
    $().invoke(url,{},
        [function(m,r){
            if(r==null){
                $("#showErrorCate").show();
                $("#PrimaryCategoryshow").html("");
                $("#showErrorCate").text("请确认分类ID输入无误！");
            }else{
                $("#showErrorCate").hide();
...
//得到分类名称
function getSecondaryCategoryName(categoryId,siteId){
    if(categoryId!=null&&categoryId!=""&&categoryId!=undefined){
        var url=path+"/ajax/getCategoryName.do?categoryId="+categoryId+"&siteId="+siteId;
        $().invoke(url,{},
            [function(m,r){
                if(r==null){
                    $("#SecondaryCategoryshow").html("");
                    $("#showErrorCate").text("请确认分类ID输入无误！");
                }else{
                    $("#SecondaryCategoryshow").html(r);
                }
            },
                function(m,r){
                    alert(r);
...
function addSecondaryCatename(obj,site){
    var categoryId = $(obj).val();
    if(site==""){
        site = $("select[name='site']").find("option:selected").val();
    }
    getSecondaryCategoryName(categoryId,site);
}
//当输入分类，
...
function selectType(obj) {
    ...
    CategoryType = $.dialog({
        id:"itemClass_",
        title: '选择商品分类',
        content: 'url:' + path + '/category/initSelectCategoryPage.do?doType='+obj,
        icon: 'succeed',
        zIndex:2000,
        width: 650,
        lock: true
...
function queryType(obj) {
    ...
    CategoryType = $.dialog({
        id:"itemClass_",
        title: '搜索商品分类',
        content: 'url:' + path + '/category/initQueryCategoryPage.do?title='+title+'&doType='+obj,
        icon: 'succeed',
        zIndex:2000,
        width: 850,
        height:600,
...
function templatePicShow(urls) {
    ...
    for (var i = 0; i < urls.length; i++) {
        var imgsrc = urls[i].src.replace("http@", "http:");
        str += '<li><div style="position:relative"><input type="hidden" name="blankimg" value="' + imgsrc + '">' +
            '<img src=' + chuLiPotoUrl(imgsrc) + ' height=\"80px\" width=\"78px\" />' +
            ...
            [function (m, r) {
                for (var i = 0; i < r.length; i++) {
                    if(i==0){
                        ...
                        preload_data[i+1] = { id: r[i].id, text: r[i].sku, text1: r[i].sku};
                    }else{
                        preload_data[i+1] = { id: r[i].id, text: r[i].sku, text1: r[i].sku};
                    }
                }
